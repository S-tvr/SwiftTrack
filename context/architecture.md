...
**B** is preferred because the only places this arises are `SettingsPage` (a `<select>` of 20–25, whose values we control) and `EmployeeForm`'s `hourlyRate` — both of which are cleaner as plain numbers than as coerced strings, and neither needs the wider generic signature bleeding into every consumer of the form.

⚠️ **A second zod trap, from the same family and found in the step-13-3 review: `.trim()` must come *before* `.min(1)`, and putting it after fails silently.** Measured on zod 4.5.1:

| Schema | `"   "` |
|---|---|
| `z.string().trim().min(1)` | **rejects** (and `" ok "` parses to `"ok"`) |
| `z.string().min(1).trim()` | **passes, yielding `""`** |

Length is checked in written order, so the reversed form measures the untrimmed string and then trims — producing a valid-looking empty value. The same applies to trimming at the **call site** instead: `EmployeeForm` validated with `.min(1)` and trimmed inside `submit()`, so a name of spaces passed the client and reached the API as `""`. It came back as a `ValidationPipe` 400, which carries **no `code`** and therefore reads as the generic *"Something went wrong."* — the worst possible message for a field the user can see they filled in.

⚠️ `z.email()` is the exception that proves the shape of the rule: its format check runs **before** any `.trim()`, so `" a@b.local "` is rejected either way and adding one there does nothing.

**The rule: every string transform belongs in the schema, ahead of the constraint it feeds.** `zodResolver` hands the submit handler the **parsed** output, so the schema is the one place that both validates and normalises — and a trim at the call site guarantees that what was validated is not what gets sent.
...
│   ├── prisma/
│   │   ├── schema.prisma                  → User, UserRate, TimeEntry, AppSettings, AuditLog models
...
│   │   │   └── prisma.module.ts
│   │   ├── audit/                       → the audit trail (step 18)
│   │   │   ├── audit.module.ts         → exports AuditService· imports NO PrismaModule, deliberately
│   │   │   ├── audit.service.ts        → record()/recordAll(), both taking the caller's tx
│   │   │   ├── audit-entry.types.ts    → the allowlist, as a discriminated union
│   │   │   └── audit.service.spec.ts
│   │   ├── auth/
│   │   │   ├── auth.module.ts
│   │   │   ├── auth.controller.ts         → POST /auth/login, /auth/set-initial-password (both behind ThrottlerGuard)· PATCH /auth/change-password (JwtAuthGuard instead, @SkipThrottle — step 8e)
│   │   │   ├── auth.service.ts            → login, setup-code verification, change-password, hashing — uses UsersService, never Prisma directly
│   │   │   ├── jwt.strategy.ts            → also rejects a stale tokenVersion (step 8f)
│   │   │   ├── jwt-auth.guard.ts
│   │   │   ├── jwt-payload.interface.ts   → { userId, role, tokenVersion } — what is signed, and what req.user holds
...
│   │   │   └── dto/                       → login, set-initial-password, change-password (step 8e), change-password-response (step 8f), login-response
│   │   ├── users/
│   │   │   ├── users.module.ts
...
State: the employee list, the "show deactivated" toggle, the create/edit dialog, the setup-code dialog, and the employee awaiting deactivation.
Logic: `GET /users` returns EMPLOYEE rows including deactivated ones. Three badge states from the pair `isActive`/`hasActivated` — a deactivated employee has a password, so a two-badge design would show them as "Active". Create → `POST /users`, then the code dialog. `PUT /users/:id` for name/hourlyRate — a **changed** rate applies from the next cycle, which `EmployeeForm` states in a permanent line (edit mode only) rather than a toast. `DELETE` (deactivate, confirmed) and `PATCH /users/:id/reactivate`. `POST /users/:id/reset-setup-code` from a pending row. The page owns all six writes, as `SettingsPage` and `ShiftHistoryPage` do.
Contains: `EmployeeList`, `EmployeeForm`, `DeactivateEmployeeDialog` (mirrors `DeleteShiftDialog`), `SetupCodeDialog` (one component, two call sites), the toggle **with its count**, and the code with its expiry **date**. Clicking a row → `/shifts/:userId`, deactivated rows included, since their history is what deactivation preserves.

⚠️ **A fourth state exists behind the three badges, and `isPending()` is what the UI keys off.** `isActive: false, hasActivated: false` is reachable, and measured against the running backend in 13-3: `deactivate` leaves `setupCode` intact, `set-initial-password` checks `isActive` *before* the code (so that code returns `401 ACCOUNT_DEACTIVATED`), and `reset-setup-code` refuses only an already-*activated* account — returning `200` and a fresh, equally dead code on a deactivated one. Such a row therefore shows **no code and no "New code"**. `isPending()` in `api/users.ts` is `isActive && !hasActivated` and is the single implementation· `!hasActivated` alone is the bug.

⚠️ **Two writes have no dialog and no form**, which is new: `Reactivate` and `New code` fire straight from a row button, so `toast.error` carries their failures. It is also the only route by which `SCREEN_ERRORS.team` reaches a user — see § Invariants.
...
Contains: a single Base UI `Select` of 20–25 bound through `Controller`, the derived line beside it, a permanent line saying what moving the boundary does to past cycles, and `ChangeCycleDialog` — which mirrors `DeleteShiftDialog` exactly rather than being a second confirmation pattern.
...
every employee in ONE request (5 queries total, not 5 per person — settings,
employees, the rates in force, closed shifts, open shifts), so the overview row
and that employee's own page can never disagree.
```
...
POST /auth/login → AuthService validates bcrypt hash → JWT { userId, role, tokenVersion }, valid 12 hours
        ↓
...
async getPayrollForCycle(userId: number, cycle?: string) {
  ...
  // ⚠️ This MUST come first: the rate is resolved *as of* the cycle's start, so
  // there is nothing to look up until the boundary is known.
  const { range, cycleDto } = await this.settingsService.resolveCycleRange(cycle);

  // Through UsersService, never prisma.user/prisma.userRate directly — those
  // models have one owner (see Invariants). And through a reader narrow enough
  // to answer only this question, so password/setupCode are never loaded to
  // compute a wage. A non-EMPLOYEE id (the admin's own included) → null → 404.
  // `range.start` is what leaves an already-priced cycle alone when the employee
  // is later given a raise.
  const employee = await this.usersService.findEmployeeRateAt(userId, range.start);
  ...
  // ⚠️ buildDayZoneHours also applies the OVERTIME threshold (173.33 h/cycle),
  // walking the shift pieces in CHRONOLOGICAL order and cutting in milliseconds
  // before any cell is rounded. It lives inside this function rather than in the
  // service so that /payroll/:userId and /payroll/overview cannot be given
  // differently-priced cycles — there is no second path to forget.
...
`findEmployeeRateAt()` in a loop, which would turn fifteen employees into thirty
round trips on a page that should cost one. Rates are batched the same way:
`findAllEmployeeRatesAt(range.start)` issues **two** queries — the employees,
then every rate row at or before that instant — and folds them in memory,
ascending, so the last row per user is by construction the one in force.
Deliberately not `DISTINCT ON`: that means `$queryRaw`, which would be the first
raw SQL here and would lose the explicit `select` this codebase applies
everywhere, to save nothing on a table whose whole history is a few dozen rows.

⚠️ The two endpoints now reach the rate through **different readers**, which is a
way for them to disagree that a shared `summarise()` does not cover on its own —
it guarantees the same arithmetic, not the same rate going into it. Both are
asked for `range.start`, and an e2e test holds them to it.
...
- `TimeEntry.endTime = null` entries are never included in payroll totals — only "closed" shifts count. This holds for the overtime threshold too: an open shift pushes nobody over 173.33, because it contributes no hours at all until it is closed.
- **`resolveZone()` never returns `OVERTIME`** *(step 16)*, and must not be changed to. The other four zones are a function of one instant — the clock decides them — while overtime depends on **how much was already worked earlier in the cycle**, which an instant cannot answer. It is applied downstream, in `buildDayZoneHours`, by walking the shift pieces in **chronological order** and cutting in milliseconds **before** any cell is rounded.
  - ⚠️ The chronological walk is load-bearing, not tidiness. This is the first calculation in the system where **the order of the hours changes the money**: before overtime, every `(date × zone)` cell was priced independently and the input order was irrelevant. Prisma returns shifts in no guaranteed order, so the sort by `startMs` is what makes the wage deterministic. A unit test pins it by feeding the same shifts reversed.
  - ⚠️ Do not "simplify" this into capping the already-aggregated day cells in `PAY_ZONES` order. That was measured during step 16's design and is **wrong by up to 8 hours per cycle**: display order puts `DAY` first, while within a date the hours actually run NIGHT → DAY → EVENING. Capping cells in *chronological zone* order is merely less accurate (0.07 h vs 0.04 h against a minute-by-minute reference) and, worse, silently correct only for as long as zones stay defined by fixed hour boundaries.
...
- **`PATCH /auth/change-password` acts only on the caller's own row** *(step 8e)* — `userId` comes from the JWT via `@CurrentUser()`, never from the body, mirroring the rule already applied to every user-scoped service function. It is the one gap `login`/`set-initial-password` left open: neither activation nor deactivation gives an already-activated account a way to rotate or recover its password short of a database edit. `AuthService.changePassword()` reaches `User` only through `UsersService.findCredentialsById()` (a narrow reader — `select: { id, password, role }`, never a general `findById()`· `role` joined it in step 8f only because the replacement token must carry one) and `UsersService.updatePasswordAndRevokeTokens()`, the same pattern every other cross-module read already follows. ⚠️ **Not rate limited** (`@SkipThrottle()`, explicit rather than merely absent): unlike `login`, a request here already requires a valid token, so brute-forcing `currentPassword` needs a stolen session first, not just a guessable email — the asymmetry is deliberate, not an oversight. A **new** code, `INVALID_CURRENT_PASSWORD`, is distinct from `INVALID_CREDENTIALS` on purpose: the caller here is already authenticated, so there is no account to enumerate by separating them.
  - ⚠️ **It refuses with `400`, never `401`** *(step 8f, and not to be moved back)*. `api/client.ts` logs the user out on **any** 401 that carried an `Authorization` header — a deliberate rule keyed off the header rather than a list of endpoints someone must maintain — and this is the one auth route that must send that header. Answering 401 threw the user out of the app for a typo in `currentPassword` and told them their session had expired. `login` and `set-initial-password` escape the same trap only because they pass `auth: false`. The route's 401 is now exclusively guard-level.
  - **It revokes every token issued before the call** *(step 8f)*, reversing what step 8e recorded as an accepted gap: `updatePasswordAndRevokeTokens()` bumps `tokenVersion` in the same `UPDATE` as the hash. Since that kills the caller's own token too, the route returns `{ accessToken }` — the session that made the change is the one kept alive, every other device is out on its next request. A second new code, `NEW_PASSWORD_SAME_AS_CURRENT`, refuses a no-op change· the check is a plain string comparison rather than a second `bcrypt.compare`, because `currentPassword` has already been verified against the stored hash one line above.
- **`POST /users/:id/reset-password` acts on an EMPLOYEE row chosen by an ADMIN, never the caller's own** *(step 8g)* — the mirror image of `PATCH /auth/change-password`: that route needs the current password and can only ever touch the caller's own row· this one needs no password at all and can only ever touch someone else's, since `findEmployeeByIdOrThrow()` — the same lookup `PUT`/`DELETE`/`reactivate`/`resetSetupCode` already use — takes an ADMIN row out of reach the same way it always has. It closes the gap neither `change-password` (needs the old password) nor `reset-setup-code` (refuses once a password exists, by design) can reach: an already-activated employee who has forgotten their password entirely.
  - **Deliberately has no guard on activation or active state**, unlike `resetSetupCode`'s `409 ACCOUNT_ALREADY_ACTIVATED`. It succeeds on an activated row (its purpose), a still-pending row (overlapping `resetSetupCode`'s effect on purpose — refusing here would just point the admin at the other endpoint for no safety gained), and a deactivated row (also succeeds, and does **not** implicitly reactivate — `login`'s existing `ACCOUNT_DEACTIVATED`-before-`ACCOUNT_NOT_ACTIVATED` ordering already makes the reset inert until a separate `reactivate` call, the same "fourth state" `resetSetupCode` already has on a deactivated-but-never-activated row).
  - **Bumps `tokenVersion`**, reusing step 8f's revocation mechanism rather than a new one: nulling a password because it was forgotten (or is suspected compromised) is at least as strong a reason to kill existing sessions as a voluntary change is — arguably stronger, since the account holder did not initiate it. Unlike `changePassword`, there is no replacement token to return: the caller is the admin, not the employee, and the employee has no session to preserve through this call by construction — their only way back in is the fresh `setupCode`, handed over out of band exactly as at account creation.
  - ✅ **Audit trail — the gap is CLOSED (step 18).** ~~Nothing records which admin reset which employee's password, or when.~~ `POST /users/:id/reset-password` now writes a `PASSWORD_RESET_BY_ADMIN` row naming the acting admin, the employee, and the instant — in the same transaction as the reset, so the two cannot come apart. The history of the sentence is kept rather than deleted, because it is the clearest example in this file of a gap recorded honestly and then closed: it was an accepted gap through 8g, **narrowed but not closed by step 17** (operational logging made a *failed* reset traceable while a **successful** one still wrote nothing, the logger being deliberately silent on 2xx and never reading a body), and closed by step 18, which is the different artefact with the different reader that step 17's note said was still required.
  - **Not rate limited, and deliberately not `@SkipThrottle()`'d either** — considered and found not to apply, unlike `change-password`'s explicit opt-out. There is no guessable secret in this route's input: `:id` is a sequential integer, not a credential, and a wrong one only 404s.
...
- **There are exactly three rounding points in the payroll pipeline, and no others** (spec §4 decision 5d): (1) hours, to 2 decimals, per **cell** — one date × one zone, the finest figure ever displayed· (2) never on a zone's rate· (3) a zone's **pay**, to whole ISK. ⚠️ **One rate per cycle is what holds this at three.** Any proposal to prorate a mid-cycle raise partitions the cycle by rate period and prices each part separately: four money roundings become eight, and the summary's single `rate` per zone becomes either two rows or a blended figure that no longer reproduces its own Pay column. A prorating design must confront that cost up front rather than discover it. Everything above a rounded value is an exact **sum** of rounded values — `totalHours` sums the cells, `totalPay` sums the four zone amounts, `totalCost` sums the employees' `totalPay`. `totalPay` is therefore never itself rounded: rounding it again is exactly what would make the Pay column stop adding up to it.
...
- The four rate zones and their percentages are **hardcoded constants** in `rate-zones.util.ts`, never `AppSettings` fields. Payroll is recomputed on every request and never frozen, so an admin-editable percentage would silently rewrite every past cycle· changing one is a developer action requiring a deploy. ⚠️ **Do not reason from `hourlyRate` to these.** A rate is admin-editable and safe to be, because it carries history (`UserRate`, invariant below) — the zone factors carry none, so the *only* thing protecting past cycles from them is that nobody can change one without a deploy. Making a factor editable means giving it a history table first, not adding a form field.
...
- `cycleStartDay` is restricted to **20–25** and `cycleEndDay` must be exactly `cycleStartDay - 1` (19–24). ⚠️ **A business rule** — a pay cycle starts at the end of the month — narrowed from 11–25 on 2026-09-12. Enforced at three layers, each of which catches what the one above it cannot: a restricted `<select>` in the admin UI makes the mistake impossible *by accident*· `class-validator` on `UpdateSettingsDto` makes it impossible for *any caller*, Swagger and curl included· and two DB `CHECK` constraints (`AppSettings_cycleStartDay_range`, `AppSettings_cycleEndDay_derived`) make it impossible for a row **written directly to the database**, which is the one path the DTO cannot see. The same layering as `findByEmail()` + `P2002` in `createEmployee`, and as the `AppSettings` singleton check. `SettingsService.assertUsableCycleStartDay()` stays regardless: it now guards only rows predating those constraints, and it is the reason a corrupted day fails loudly rather than computing a wrong boundary. Two properties follow and are the reason for the range: consecutive cycles are **contiguous**, so no shift falls into a gap between cycles or into two of them at once· and every allowed day exists in every month, so day-of-month **clamping is never needed** for 28/29/30/31-day months. Because the range is fully determined by `cycleStartDay`, the arithmetic reads only that field — `cycleEndDay` is stored and validated but never computed with, so the two can never disagree about where a boundary is.
...
- The backend is the single source of truth for pay-cycle date boundaries. `resolveCycleRange()` lives only in `SettingsService`, which owns the `AppSettings` row it derives from; `TimeEntriesService`, `PayrollService` and `UsersService` inject `SettingsService` rather than resolving cycles themselves, the same way `AuthService` goes through `UsersService` for every `User` query. Three entry points, all in that one service, all reading `cycleStartDay` and nothing else: `resolveCycleRange()` (which cycle is this), `resolveWritableCycleStart()` (one cycle back — what an employee may still edit) and `resolveRateEffectiveFrom()` (one cycle forward — when a new rate starts applying). They are siblings on purpose: three copies of "where does a cycle begin" in three services is how the write window and the rate window would come to disagree. The date arithmetic itself lives in `cycle.util.ts` as pure functions (`cycle`, `cycleStartDay` → `{ start, endExclusive }`) with no DB and no Nest, so it can be unit-tested standalone. Every response that involves a cycle (time-entries list, payroll breakdown) returns the same block: `cycle`, `prevCycle`, `nextCycle`, `cycleStart`, `cycleEnd`. The frontend never computes cycle boundaries itself — the ◀▶ `CycleNavigator` sends back the `prevCycle`/`nextCycle` key it was handed rather than doing month arithmetic, so not even a month rollover is implemented twice.
...
- **A service that needs data from another module's model gets a narrow, purpose-named reader with an explicit `select` — never a general getter returning a whole row.** `UsersService` remains the single owner of `User` queries, so `TimeEntriesService` and `PayrollService` go through it (as `AuthService` already does), but what they get back is scoped to the question they asked: existence, or a rate. `findActiveById()` established this in step 3 for the same reason it matters generally — a method returning the full `User` carries `password` and `setupCode` to every future caller, and this project has already leaked exactly that twice (the removed `findById()` in step 2, the reused response DTO in step 3). Keeping the shape narrow makes the leak impossible rather than merely absent. The roster: `findActiveById`, `assertEmployeeExists`, `findEmployeeNameOrThrow`, `findEmployeeRateAt`, `findAllEmployeeRatesAt`, `findCredentialsById`.

- **`UserRate` is owned by `UsersService` exactly as `User` is, and a cycle's rate is resolved once — at `range.start`.** `PayrollService` never queries `prisma.userRate`; it asks through `findEmployeeRateAt(id, at, until)` or `findAllEmployeeRatesAt(at)` and gets back the same shape it would have got from a plain rate column, which is why `summarise()` and `requireHourlyRate()` know nothing about any of this. Four parts are load-bearing:
  - **Once per cycle, never per day or per shift.** The rate is a property of the cycle being priced, not of the hours inside it. Resolving it per cell would multiply the rounding points (see above) and leave the Rate column unable to reproduce the Pay column beside it.
  - **`at` is always `range.start`, and it comes from `SettingsService`.** `UsersService` never resolves a cycle and never reads `AppSettings` — it receives an instant. That is what lets cycle arithmetic stay in one module while `User` keeps one owner. ⚠️ It also fixes an ordering constraint inside `getPayrollForCycle`: the cycle must be resolved **before** the employee is read, because there is no rate to look up until the boundary is known.
  - **`User.hourlyRate` and the newest `UserRate` row are written in one `prisma.$transaction`**, justified the same way `updatePasswordAndRevokeTokens()` puts the hash and the token bump in one `UPDATE`: two writes that must never be observable apart. ⚠️ **This said "the codebase's only `$transaction`" until step 18 and is no longer true** — every mutation now opens one to carry its audit row, so there are thirteen. ⚠️ It also changed **shape** here: the array form cannot express "record what this upsert just replaced" (those promises are built before any of them runs), so it is now the callback form. The atomicity is identical; only the shape moved.
    - ⚠️ **The API field `hourlyRate` means "what they are paid now", and is resolved from `UserRate` — never served from the column** *(restored after Step 15b; the code had drifted from this)*. The **column** is the newest rate *entered*, written the instant an admin saves, while the `UserRate` row it pairs with is dated to the next cycle. So during a queued raise the two legitimately differ, and serving the column made `/users`, `/users/me` and the login response all report a figure nobody is paid yet — an employee seeing a larger rate on their profile than on their own payroll, the exact symptom spec §5g exists to prevent. `findRatesNow()` resolves both halves (in force, and queued) from one query; `toResponseDto`/`toProfileDto` receive them and never read the column.
    - The column's remaining job is `rateChanged` in `updateEmployee`: "did *this request* change the rate?" is a comparison against what was last **entered**, which is what the form submits back. ⚠️ Comparing against the in-force rate instead would make an unchanged re-save of a queued figure look like a change and upsert the identical row every time.
  - **A queued raise is reported, never left implicit.** Both `GET /users` and `GET /payroll` carry `pendingRate`/`pendingRateEffectiveFrom` — set and cleared together, like `setupCode` and its expiry. Without them the Team list shows a rate no current payslip uses and an employee sees a bigger number on their profile than on their own payroll, which reads as an underpayment rather than as a raise that has not started. ⚠️ The two endpoints answer against **different reference points**, deliberately: the Team list has no cycle, so pending there is relative to *now*· payroll is relative to *the cycle being viewed*. Neither is derivable from the other, which is why both DTOs carry the pair rather than one screen borrowing the other's call.
    - ⚠️ **Payroll announces a raise on exactly one cycle: the one it lands at the end of** *(step 15b)*. `findEmployeeRateAt` takes the cycle's `endExclusive` as `until` and matches `effectiveFrom === until`, not `> at`. The looser test was the shipped behaviour until 15b and was wrong in a way that only showed up once demo data had a raise in it: **every** cycle before a raise announced it, so paging ◀ carried the same "their rate changes to X from Y" line — future tense — across cycles that were closed and already paid. The notice exists to explain the gap between *what I am paid now* and *what this cycle was priced at*· a settled cycle two raises back has no such gap, and a reader has no decision to make about it. Equality is exact rather than lucky: `effectiveFrom` is always a cycle-start instant (see `schema.prisma`) and `endExclusive` **is** the next cycle's start, so the two coincide by construction — and `resolveRateEffectiveFrom()` only ever writes the next cycle's start, so the ordinary raise path always matches. ⚠️ The one case it does not is §13 gap 8: move `cycleStartDay` while a raise is queued and its instant lands mid-cycle, where equality finds nothing and the notice goes **silent** rather than appearing a cycle late. Known, recorded there, and not fixed here.
  - **`toResponseDto` takes the resolved rates as an argument and never looks them up.** It is a pure synchronous mapper with eight call sites, one of which maps over the whole team — querying inside would make `GET /users` cost one round trip per employee. Callers fetch them, batched where they can (`findRatesNow` folds one `findMany` into a `Map`, the same shape as `findAllEmployeeRatesAt`). ⚠️ The parameter is **required, with no default**: a default would silently restore the served-from-the-column bug in any call site added later, whereas a required one makes the compiler name every one of them.
  - **Both halves of a rate — in force and queued — come from one query, read against one `now`.** `findRatesNow` fetches the whole history for the ids it is given and partitions it in memory, rather than issuing two filtered queries. Not an optimisation: the rate on screen and the raise announced beneath it must be answered at the same instant, and two queries with two `new Date()`s let a cycle boundary fall between them and pair a fresh rate with an announcement that has already happened. It is also what keeps `GET /users` at two queries for the whole team.
  - **Every employee's first rate row starts at the epoch**, written by `createEmployee` in the same statement as the user. An admin may write a shift at any past date, so a cycle earlier than the employee's own row is reachable — and a cycle with no rate in force throws, which on the overview takes down the page for the whole team. The epoch anchor keeps that 500 meaning "somebody edited the database by hand" instead of "somebody was hired in August".
...
- `POST /auth/login` returns `{ accessToken, user }` — the token alone is not enough for the client, since the JWT payload carries only `{ userId, role, tokenVersion }` and the `Header` needs `name` on every protected page. Returning the user here is what spares the frontend a follow-up `GET /users/me` on every login.
- The JWT is a single token valid for **12 hours** *(shortened from 14 days in step 8f)*, with **no refresh-token mechanism** — that remains deliberately out of scope for Phase 1, since it would require a stored, rotated credential, a `/auth/refresh` route, and silent-retry logic in `api/client.ts` that de-duplicates concurrent refreshes. Half a day is roughly one login per shift in a shift-tracking app, and it bounds a stolen token to hours rather than weeks. ⚠️ **Revocation, however, is no longer absent** — see the `tokenVersion` invariant below. The two are orthogonal: the expiry bounds *every* token whether or not anyone asks, while `tokenVersion` fires at the moment a user asks for protection.
- **The token proves identity; the database decides authority.** `JwtStrategy.validate()` resolves the payload's `userId` through `UsersService.findActiveById()` on every authenticated request and rejects the request if that user is missing, `isActive` is `false`, or the row's `tokenVersion` no longer matches the token's. So `DELETE /users/:id` takes effect immediately instead of leaving a deactivated user working until their token expires. `role` and `tokenVersion` are read from the row for the same reason: they are current, not whatever was signed. The lookup uses `select` to fetch only `id`, `role` and `tokenVersion`; running on every request, it must never pull `password`/`setupCode` into memory. This deliberately replaces the original "trust the payload, never touch the DB" decision from Step 3, which was made when every endpoint was read-only — Time Entries writes hours that become wages, and a per-endpoint check would be a rule someone must remember on each new write route.
- **`tokenVersion` is what makes revocation possible at all, and it costs nothing** *(step 8f)*. It is an `Int @default(0)` on `User`, signed into every token and compared against the row on every request — inside the `findActiveById()` read that the invariant above already performs, so revocation adds **no query**. Exactly one place increments it: `UsersService.updatePasswordAndRevokeTokens()`, in the same `UPDATE` that stores the new hash, so the two can never diverge. **`setInitialPassword` deliberately does not** — an unactivated account cannot have issued a token, so there is nothing to revoke — and **neither do `deactivate`/`reactivate`**, since `isActive` is already enforced live by the same lookup. ⚠️ Because a bumped counter invalidates the caller's *own* token too, `PATCH /auth/change-password` returns a replacement in its body: the session that performed the change is the one session that survives. ⚠️ Any deploy that introduces this field logs every existing session out once, since tokens signed before it carry no `tokenVersion` at all.
...
- **Nothing a request carries is ever written to a log — no body, no headers** *(step 17)*. `FailedRequestMiddleware` writes method, URL, status, duration and the caller's `userId`, and reads nothing else off the request. ⚠️ This is the **absence of a path**, not a filter over one, and the distinction is the whole design: four endpoints take a secret in their body (`/auth/login`, `/auth/set-initial-password`, `/auth/change-password`, `POST /users`), every authenticated request carries a 12-hour bearer token, and an allowlist of "safe" fields would be a list to maintain at each new DTO whose failure mode is silent — the secret simply appears in the log and nothing says so. With no read there is nothing to leak. This is what enforces the older "passwords are never logged in plaintext" invariant by construction rather than by care. Two corollaries: the log line carries **no domain `code`**, because obtaining one means reading the response body (the same closed path) — status plus route is enough to find the request; and `req.user` is narrowed to `userId` alone, never spread, since it also holds `tokenVersion`, a revocation counter with no business in a log.
- **`AllExceptionsFilter` only ever reads — every branch ends in `super.catch()`** *(step 17)*. It reopens the alternative `domain-errors.ts` recorded as rejected, and the reason it was rejected does not apply: what was refused is a filter that *decides* the body (needing a status → code map, "precisely the collapse these codes exist to undo"). This one never touches it, so the response is byte-for-byte what it was — pinned by `logging.e2e-spec.ts`, because a claim like that is worth what its test is worth. ⚠️ The gap it closes is narrower than "log errors": Nest's own `BaseExceptionFilter` already logs anything that is **not** an `HttpException`, so what was invisible was an `HttpException` carrying a 5xx — exactly the three deliberate `InternalServerErrorException`s (missing `AppSettings`, out-of-range `cycleStartDay`, `null hourlyRate`), each of which names its own fix in a message no reader could reach. ⚠️ **4xx is deliberately silent here**, for the hazard rather than the tidiness: `ValidationPipe`'s 400s put the **offending values** in `getResponse()`, so anyone extending this filter to 4xx must solve that first — the safe field is the status, never the response object. ⚠️ **A 5xx does produce two lines** (this filter's, carrying the stack· the middleware's, carrying the duration), and that is deliberate: neither can answer the other's question, since a filter is handed an exception rather than the start of a request. The stack — the expensive half — is written exactly once, and a unit test pins the pair. An earlier draft of this invariant claimed "one event, one line" and was **measured wrong** during the step 17 review· it is corrected here rather than quietly dropped, because it is the same doc/code divergence the step 16 review caught twice. That silence also avoids a real hazard — `ValidationPipe`'s 400s put the **offending values** in `getResponse()`, so anyone extending this filter to 4xx must solve that first; the safe field is the status, never the response object.

- **Every successful mutation writes an `AuditLog` row, in the same transaction as the mutation itself** *(step 18)*. Closes spec §13 gap 2 and the half of 8g's gap no single endpoint could close. ⚠️ **Same transaction, never best-effort**: if the audit insert fails the change rolls back with it, so a change with no trace is unreachable rather than merely unlikely — the request failing loudly is the designed outcome, and an e2e test forces it with a trigger that refuses the insert. This is what `AuditService.record(tx, entry)` enforces structurally: it takes the caller's `Prisma.TransactionClient` and `AuditModule` imports no `PrismaModule`, so there is no client available with which to write outside the caller's transaction. The cost was booked knowingly — 12 of the 13 write paths had no transaction at all and now open one.
  - ⚠️ **Cost booked rather than discovered: two pool connections per write.** Six `UsersService` methods run `writeAndAudit` inside a `Promise.all` alongside `findRateNow`, so a transaction and a query are held at once against the `pg` adapter's default pool of 10 — roughly five concurrent writes of this shape before it saturates. Measured as adequate for this project's scale (one install, few users, transactions lasting milliseconds) and **deliberately not "fixed"**: serialising them would cost a guaranteed extra round trip on six endpoints to buy a limit nobody is near. It is recorded here so a future headcount change finds the number rather than rediscovers it — the fix then is an explicit `connection_limit`, not a reshaping of these call sites.
  - ⚠️ **Written in the service, never a controller or an interceptor.** `AuthService` reaches `User` through `UsersService.activateAccount()` and `updatePasswordAndRevokeTokens()`, so a controller-level hook would miss both. An interceptor is worse than incomplete: it sees a request and a response, never the row as it stood **before** the write, and it cannot join the transaction.
  - ⚠️ **Clock-in and clock-out write no row, deliberately.** The `TimeEntry` *is* the record there — written by the server's own clock, from the caller's own session, with no "before" and no gap between actor and subject. The manual path (`POST`/`PUT`/`DELETE /time-entries`) is audited because those are the hours a human types. Pinned by a test so the absence reads as a decision.
  - ⚠️ **A no-op writes nothing, and the test is the *state*, never the request.** `reactivate` on an already-active row answers 200 and still issues an UPDATE (a documented double-submit path), so the audit write is skipped unless something actually changed. The same rule governs `updateEmployee`, where it is sharper: `EmployeeForm` submits **both** fields on every save, so an identical re-save is the ordinary path rather than an edge case, and `nameChanged`/`rateChanged` are computed together at the top of the method precisely so the two write branches cannot answer it differently. ⚠️ **They did, and the step 18 review caught it live**: the no-rate branch wrote an `EMPLOYEE_UPDATED` whose `before` and `after` were identical while the rate branch guarded correctly — an event that did not happen, in the one table whose value is that its rows are believable. A row is written when the state moved· the write itself always goes out, because an audit decision must never change what the endpoint answers. ⚠️ Note what the four mutation tests could **not** do here: a mutation breaks a guard that exists and cannot reveal one that is missing, which is why this needed a review and not another spike.
  - ⚠️ **Two rows, not one, where one request does two things.** `PUT /users/:id` can rename an employee *and* queue a raise; recording one event would leave a reader unable to tell which. The rate row's `before` is the only surviving copy of a **superseded** queued rate — `UserRate` upserts on `(userId, effectiveFrom)`, so a second raise inside one cycle overwrites the first and that figure exists nowhere else. Verified live.
- **`AuditLog.before`/`after` are built from an explicit per-entity allowlist, and never by spreading a row** *(step 18)*. `TimeEntry`: startTime/endTime/notes· `User`: name/hourlyRate/isActive· `UserRate`: hourlyRate/effectiveFrom· `AppSettings`: both cycle days. ⚠️ This is the **same discipline as the request-logging invariant above, one layer down**: the tempting alternative — snapshot the Prisma row, strip the secrets — is a *blocklist*, a list to maintain at every new column whose failure mode is silent (the secret simply appears in the table and nothing says so). The allowlist is expressed as types, so the compiler refuses a payload that was never declared, and the mapper functions take **narrow structural types rather than the Prisma model**, so a secret is never in scope to begin with. `password`, `setupCode`, `setupCodeExpiresAt` and `tokenVersion` are never values here — a revocation is recorded as an **action** (`PASSWORD_CHANGED`, `PASSWORD_RESET_BY_ADMIN`), never as a counter. Proved twice: a unit test hands `toEmployeeSnapshot` a full `User` row, and an e2e test runs real secrets through real endpoints and greps the whole table.
  - ⚠️ **The snapshots are `type` aliases, not `interface`s, and that is load-bearing**: Prisma's `InputJsonValue` needs an implicit index signature, which TypeScript grants a type alias and withholds from an interface. As interfaces they do not compile against `auditLog.create`, with an error ("Index signature for type 'string' is missing") that is confusing to meet from the far end.
- **`AuditLog` is append-only, enforced by a database trigger** *(step 18)* — a `BEFORE UPDATE OR DELETE` trigger raising `restrict_violation`, hand-written in the migration like the `AppSettings` `CHECK ("id" = 1)` and the partial unique index, and for the same reason: append-only is the whole claim of an audit log, and a claim held only by "no code does that today" is a convention waiting for the next writer. ⚠️ **Not `REVOKE UPDATE, DELETE ... FROM PUBLIC`, which was measured and does nothing here**: the app connects as a role that both **owns** the table and is a **superuser**, and neither consults table privileges — the UPDATE succeeded with the REVOKE in place. Row-level security fails for the same reason. ⚠️ **`TRUNCATE` is deliberately not blocked**: a row-level trigger is not consulted by it, so the guarantee is precisely that no *individual* row can be altered or removed, while emptying the table is all-or-nothing and conspicuous. That escape hatch is load-bearing — `test/helpers/db.ts` resets the table between e2e tests with it, and a retention policy would need it — so a `BEFORE TRUNCATE` statement trigger was considered and declined.
- **`AuditLog` carries no foreign key to `User`** *(step 18)*. `actorId`/`subjectId` are plain `Int?` columns: a relation makes audit rows reachable by cascade, and a trail that disappears when its subject is deleted is not a trail. It also keeps a row readable after the user row is gone. Verified live — deleting an employee left all six of their audit rows intact. The missing FK is the feature, not an oversight.
- **The actor reaches the service on every write path** *(step 18)*. `@CurrentUser()` is threaded through all six mutating `users` routes and `PUT /settings`, which previously received only the **subject** (`@Param('id')`) — so the table could have said "the rate changed" but not who changed it, which is exactly what 8g asked for. The parameter shape follows `time-entries`, which already had it: the actor comes **first** in the service signature, the subject after. ⚠️ Actor and subject are recorded even where they coincide (an employee editing their own shift), so the admin-written row and the self-written row are read the same way rather than one of them being a special case.
...
- **`frontend/src/lib/datetime.ts` is the only place a date or time is formatted or parsed.** No component calls `new Date`, `toLocaleString` or `toLocaleDateString`. Every formatter passes `{ timeZone: "UTC" }`, matching the backend end to end. A `datetime-local`-shaped value is converted by **appending `":00.000Z"`** — ⚠️ **never `new Date(value).toISOString()`**, which reads it as local time and shifts it by the developer's offset, moving the shift into a different rate zone and changing someone's pay. That bug compiles, looks right, and no backend test can see it.
  - ⚠️ `ShiftForm` no longer uses a `<input type="datetime-local">`: its popup is ~320px of browser chrome that covered the dialog's Save button at every screen height, so each end is now a **`date` + `time` pair**. The shape above is unchanged — `fromDateAndTime()` joins the halves back into the same `YYYY-MM-DDTHH:mm` string that `toIsoUtc()` has always taken, and `toDateAndTime()` splits an API instant for display. Both live in this module, as the rule requires. ⚠️ `fromDateAndTime` returns `""` when **either** half is missing rather than defaulting the other: a date with no time is not an instant, and inventing midnight would store a shift nobody entered.
...
- **A successful write is confirmed by a toast when the screen it leaves the user on cannot show that it happened** (fixed in step 13-2, so 13-3 copies a rule instead of deciding six times). This *names* what steps 10 and 11 already decided rather than adding anything: clock-in gets **no** toast because it rewrites the button and adds a line beneath it — a confirmation that is structural and permanent — while `ShiftForm` gets one because the dialog closes onto a list that may be identical. Settings is the sharpest case and the reason the rule is written down here: a save leaves the same page with the same values, so without a toast the only evidence is the Save button going quiet. Two corollaries follow for step 13-3: **deactivate takes a toast** (the row disappears behind the filter, which reads as a hard delete of someone whose payroll history is in fact kept), and **create takes none** (the setup-code dialog opens, which is louder than any toast).
  - ⚠️ **Step 13-5 adds a third corollary that sharpens the rule rather than reusing it: `Reset password` takes no toast because the write is not the end of the job.** The other exemptions rest on the screen already showing what happened· this one rests on the fact that nothing has *finished* happening. The reset leaves a code that still has to reach the employee by voice or on paper — the app has no channel for it — so success opens the code dialog, and a toast would announce completion at the halfway point. The test is therefore not only "can the screen show it?" but "is there anything left for the user to do?".
...
- **A write with no dialog and no form reports its failure through `toast.error`** (added step 13-3). Every write before it was launched from a form or a confirmation that could hold its own error: `ShiftForm` renders one above its buttons, `DeleteShiftDialog` and `ChangeCycleDialog` stay **open** and show the reason. Team introduces the first two that have neither — `Reactivate` and `New code` fire straight from a row button — and without this they fail silently, leaving a row that simply does not change. It is the mirror of the `toast.success` rule and rests on the same test: *the screen the user is left on cannot show what happened.* ⚠️ It is also the **only** path by which `SCREEN_ERRORS` can reach anyone on this page: `ACCOUNT_ALREADY_ACTIVATED` comes back from `reset-setup-code` against a stale list, and the per-screen override written in step 9 exists precisely so the admin reads "refresh the list" rather than the employee's "go and sign in". Had these two writes stayed silent, that override would have remained unreachable — the string was already three steps old and still unused.

- **An admin-facing response type is declared standalone on the client too, never derived from the self-facing one** (added step 13-3). `UserResponse` in `api/users.ts` carries `setupCode`/`setupCodeExpiresAt`/`isActive`/`hasActivated`· `UserProfile` carries none of them. Writing `Omit<UserResponse, "setupCode">` — or the reverse — would be the same leak-by-default the backend already refuses between `UserResponseDto` and `UserProfileDto`: a field added to the admin's shape flows silently into the self-facing one, and the compiler reports nothing because subtraction is the *absence* of a decision. Adding a field to either must be an act of writing it there. The duplication is the point, not an oversight.

- **Where a UI state is a compound of two flags, the predicate is a named function with one implementation** (added step 13-3). `isPending()` in `api/users.ts` is `isActive && !hasActivated`, and it governs three separate things — the setup code, its expiry, and the **New code** button. The tempting `!hasActivated` is wrong in a way nothing on screen reveals: it also matches a *deactivated* employee with no password, whose surviving code returns `401 ACCOUNT_DEACTIVATED` and for whom `reset-setup-code` cheerfully issues another (measured, 13-3). Three inline copies of a two-flag test is how one of them comes to disagree with the other two· one function is how the spike can invert it and turn exactly the right tests red.
  - ⚠️ **"Pending" means the account has no password *right now*, not that it never had one** — widened in step 8g, and the widening is deliberate rather than incidental. `POST /users/:id/reset-password` clears an activated employee's password, so `hasActivated` goes back to `false` and a fresh code appears: that row is now indistinguishable from a never-activated one, and **must be**, because the way back in is identical (set a password with the code). No fourth badge, no second code display, no branch anywhere — the predicate keeps one meaning ("waiting for a password to be set") and one implementation. The three states the badge shows are unchanged, since `statusBadge` tests `isActive` first and a reset employee is still active.
