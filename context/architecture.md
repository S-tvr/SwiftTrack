...
- **`UserRate` is owned by `UsersService` exactly as `User` is, and a cycle's rate is resolved once — at `range.start`.** `PayrollService` never queries `prisma.userRate`; it asks through `findEmployeeRateAt(id, at, until)` or `findAllEmployeeRatesAt(at)` and gets back the same shape it would have got from a plain rate column, which is why `summarise()` and `requireHourlyRate()` know nothing about any of this. Four parts are load-bearing:
  ...
  - **`User.hourlyRate` and the newest `UserRate` row are written in one `prisma.$transaction`.** `hourlyRate` means "what they are paid now" and is what `/users`, `/users/me` and the login response report· `UserRate` means "what they were paid then" and is the only thing payroll reads. The transaction is why the denormalised head is safe to trust. ⚠️ This is the codebase's only `$transaction`, justified the same way `updatePasswordAndRevokeTokens()` puts the hash and the token bump in one `UPDATE`: two writes that must never be observable apart.
  ...
  - **`toResponseDto` takes the pending row as an argument and never looks it up.** It is a pure synchronous mapper with eight call sites, one of which maps over the whole team — querying inside would make `GET /users` cost one round trip per employee. Callers fetch it, batched where they can (`findPendingRates` folds one `findMany` into a `Map`, the same shape as `findAllEmployeeRatesAt`).
