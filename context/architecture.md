...
Backend: JwtAuthGuard validates token on all protected routes
         JwtStrategy.validate() trusts the payload — no DB lookup per request,
         so a token issued before a user was deactivated stays valid until it expires
...
- The JWT is a single token valid for **14 days**, with no refresh-token mechanism — deliberately out of scope for Phase 1 (it would require storing a revocable token, a `/auth/refresh` route, and silent-retry logic in `api/client.ts`). `JwtStrategy.validate()` trusts the signed payload and performs **no DB lookup per request**. Consequence, accepted knowingly: deactivating a user via `DELETE /users/:id` does not revoke tokens they already hold — those keep working until they expire naturally. `isActive` is re-checked at login, so they cannot obtain a new one.
