...
│   │   │   └── dto/                       → login, set-initial-password, change-password (step 8e), login-response
│   │   ├── users/
│   │   │   ├── users.module.ts
...
- **`PATCH /auth/change-password` acts only on the caller's own row** *(step 8e)* — `userId` comes from the JWT via `@CurrentUser()`, never from the body, mirroring the rule already applied to every user-scoped service function. It is the one gap `login`/`set-initial-password` left open: neither activation nor deactivation gives an already-activated account a way to rotate or recover its password short of a database edit. `AuthService.changePassword()` reaches `User` only through `UsersService.findCredentialsById()` (a narrow reader — `select: { id, password }`, never a general `findById()`) and `UsersService.updatePasswordHash()`, the same pattern every other cross-module read already follows. ⚠️ **Not rate limited** (`@SkipThrottle()`, explicit rather than merely absent): unlike `login`, a request here already requires a valid token, so brute-forcing `currentPassword` needs a stolen session first, not just a guessable email — the asymmetry is deliberate, not an oversight. ⚠️ **Does not invalidate tokens already issued** — this API has no refresh/revocation mechanism (see the JWT invariant below), so a token minted before the change stays valid until its own 14-day expiry. A **new** code, `INVALID_CURRENT_PASSWORD`, is distinct from `INVALID_CREDENTIALS` on purpose: the caller here is already authenticated, so there is no account to enumerate by separating them.
