...
## Step 15b — Demo seed: employee με αύξηση
Status: ✅ Done
Date: 2026-09-09

**Μικρή συνέχεια των Step 15/15a.** Ο κώδικας ήταν σωστός και τεσταρισμένος, αλλά **κανένα demo δεδομένο δεν τον έδειχνε**: κάθε demo άτομο είχε ακριβώς **μία** γραμμή `UserRate` στο epoch, άρα όλοι οι κύκλοι τιμολογούνταν ίδια και το pending badge δεν εμφανιζόταν ποτέ. Όποιος άνοιγε το demo — ή έκανε review τη λειτουργία με το χέρι — δεν έβλεπε τίποτα από τα δύο.

### Files added/changed
- `backend/prisma/seed-demo.ts` (μόνο αυτό)

### Τι προστέθηκε
Ένα τέταρτο άτομο στο `PEOPLE` (**Ragnar Vilhjálmsson**, `ragnar@demo.local`) που κουβαλά **και τα δύο** μισά ταυτόχρονα, με 3 γραμμές `UserRate`:

| `effectiveFrom` | `hourlyRate` | τι δείχνει |
| --- | --- | --- |
| epoch | 2400 | η αρχική τιμή |
| **αρχή τρέχοντος** κύκλου | 2900 | αύξηση που **ήδη ισχύει** (Step 15) |
| **αρχή επόμενου** κύκλου | 3300 | αύξηση **σε εκκρεμότητα** (Step 15a) |

### ⚠️ Το λάθος που βρήκε ο χρήστης — και γιατί έχει σημασία
Η πρώτη εκδοχή έβαλε την ενεργή αύξηση στο `cycles[1].start` (**προηγούμενος** κύκλος) και το pending στον **επόμενο**. Αποτέλεσμα: ανάμεσά τους έμενε **ένας ολόκληρος κύκλος όπου δεν άλλαζε τίποτα**, οπότε το ίδιο μήνυμα «3300 από 25 Σεπ» εμφανιζόταν σε **δύο συνεχόμενους** κύκλους (`2026-07` και `2026-08`) και διαβαζόταν ως **κολλημένο μήνυμα**.

⭐ **Το μήνυμα ήταν σωστό· τα δεδομένα ήταν κακά.** Δεν υπήρχε bug στο `payroll.service.ts` — στον κύκλο 25 Αυγ–24 Σεπ το 3300 όντως εκκρεμούσε ακόμα. Αξίζει να σημειωθεί γιατί το πρώτο ένστικτο ήταν να ψάξω τη λογική: **σωστή λογική πάνω σε αφύσικα seed δεδομένα παράγει συμπεριφορά που μοιάζει με bug.**

Διόρθωση: η ενεργή αύξηση μετακινήθηκε στο `cycles[cycles.length - 1].start` — **διαδοχικοί** κύκλοι, ώστε η pending τιμή να αλλάζει **ακριβώς όταν** αλλάζει και η τιμή.

### Οι αποφάσεις
1. **Flag `raise?` στο `DemoPerson`**, όχι positional lookup — ακριβώς το μοτίβο που το σχόλιο των γραμμών 55-63 επιβάλλει ήδη για τα `boundaryShifts`/`openShift`.
2. **Μέσα στο `PEOPLE`**, όχι ξεχωριστό creation path — έτσι το `buildShifts` του δίνει βάρδιες **και στους τρεις** κύκλους αυτόματα.
3. **Pattern εξ ολοκλήρου στη ζώνη DAY.** Το νόημα του ατόμου είναι ότι *κινείται η τιμή του*· pattern σε τέσσερις ζώνες θα έκανε τα ποσά δυσανάγνωστα απέναντι στην αλλαγή τιμής.
4. **`hourlyRate: 2900`** στη στήλη — η τιμή που ισχύει **τώρα**, όχι η αρχική: είναι το denormalised head του ιστορικού και το `schema.prisma` απαιτεί να συμφωνεί με τη νεότερη μη-μελλοντική γραμμή. Επαληθεύτηκε στη βάση.

### ⭐ Καμία ημερομηνία δεν είναι hardcoded
Και τα δύο instants παράγονται από το `cycleStartDay`: το ενεργό από το `cycles[cycles.length - 1].start`, το pending από `computeCycleRange(shiftCycleKey(currentKey, 1), …).start` — που **αναπαράγει ακριβώς** το `SettingsService.resolveRateEffectiveFrom()`. Άρα το seeded state είναι state που **το ίδιο το API θα μπορούσε να είχε γράψει**.

Και τα δύο πέφτουν σε **cycle-start instant**, ποτέ μεσοκύκλια: το `schema.prisma` το τεκμηριώνει ως invariant της στήλης, και μεσοκύκλια τιμή θα έσπερνε κατάσταση που το API δεν παράγει — η ίδια κατηγορία προβλήματος με το σχόλιο των γραμμών 143-149 για τις βάρδιες.

### ⚠️ Λάθος υπόθεση που διορθώθηκε κατά τον σχεδιασμό
Η αρχική ανάγνωση του αιτήματος ήταν ότι **ο τρέχων κύκλος δεν έχει βάρδιες**. Ελέγχθηκε: **δεν ισχύει**. Με `cycleStartDay=25`, ο τρέχων κύκλος είναι `2026-08` (25 Αυγ → 25 Σεπ) και όλο το `PEOPLE` έχει βάρδιες εκεί. Ο leaver έχει βάρδιες **μόνο** στον τρέχοντα σκόπιμα: το `payroll.service.ts:164` κόβει απενεργοποιημένο employee χωρίς ώρες, άρα βάρδιες σε παλιούς κύκλους είναι ακριβώς αυτό που θα τον **εξαφάνιζε** από το overview. Το roster έμεινε ανέπαφο.

### Verification που εκτελέστηκε
- `npm run seed:demo` **δύο φορές** → ίδιο αποτέλεσμα, κανένα unique-constraint violation στο `(userId, effectiveFrom)`. Το υπάρχον `userRate.deleteMany` καλύπτει και τις τρεις γραμμές.
- **Οι 3 γραμμές στη βάση**: epoch/2400, `2026-08-25`/2900, `2026-09-25`/3300 — όλες σε 25άρα.
- ⭐ **`GET /payroll/me` στους 3 κύκλους** — κάθε κύκλος λέει πλέον **κάτι διαφορετικό**:
  - `2026-06` (25 Ιουν–24 Ιουλ) → **2400**, pending 2900 από 25 Αυγ
  - `2026-07` (25 Ιουλ–24 Αυγ) → **2400**, pending 2900 από 25 Αυγ
  - `2026-08` (25 Αυγ–24 Σεπ) → **2900**, pending **3300** από 25 Σεπ
- ⚠️ Οι ώρες διαφέρουν ανά κύκλο, άρα το σκέτο `totalPay` **δεν** αποδεικνύει τίποτα. Η διαίρεση το κάνει: **2400 / 2400 / 2900** ανά ώρα — η διαφορά είναι η **τιμή**, όχι το πλήθος βαρδιών.
- **`GET /users` ως admin**: το badge **μόνο** στον Ragnar (2900 → 3300), `null` σε όλους τους άλλους — το flag δεν διέρρευσε στο roster.
- Backend unit **241/241**, e2e **127/127** — **αμετάβλητα**, όπως αναμενόταν: το e2e τρέχει σε δική του βάση που το script απαγορεύεται να αγγίξει, και κανένα test δεν κάνει assert σε demo data.
- `tsc --noEmit` και `eslint` καθαρά.

### ⚠️ Open, αμετάβλητα από το 15a
1. **13b (Playwright)** — παραμένει το τελευταίο ανοιχτό του build plan.
2. **§13 gap 7** (ενεργοποιημένη τιμή δεν διορθώνεται μέσω API) και **gap 8** (μετακίνηση ορίου ολισθαίνει εκκρεμή αύξηση).

**Next step**: **13b (Frontend E2E, Playwright)**.

---

## Step 15b — Demo seed με αύξηση + διόρθωση διαρροής του pending notice
Status: ✅ Done
Date: 2026-09-09

Ξεκίνησε ως απλή προσθήκη στο demo seed (ένας employee με αύξηση, ώστε να **φαίνεται** το Step 15) και **αποκάλυψε πραγματικό bug** στη λειτουργία.

### ⭐ Το bug
Το `findEmployeeRateAt` δήλωνε ως pending **οποιαδήποτε** μελλοντική γραμμή:

```ts
const upcoming = rates.filter((rate) => rate.effectiveFrom > at);
```

Άρα βλέποντας τον Ιούνιο, μια αύξηση με ισχύ τον Σεπτέμβρη αναφερόταν κανονικά — **τρεις συνεχόμενοι κύκλοι** κουβαλούσαν την ίδια γραμμή «their rate changes to X from Y», σε **μέλλοντα χρόνο**, πάνω σε κύκλους **κλειστούς και ήδη πληρωμένους**.

Το ίδιο το σχόλιο του μηνύματος (`messages.ts:513-518`) λέει ότι υπάρχει για να εξηγήσει τη διαφορά «τι παίρνω τώρα» vs «πώς τιμολογήθηκε αυτός ο κύκλος». **Σε κλειστό κύκλο δύο αυξήσεις πριν δεν υπάρχει τέτοια διαφορά** — άρα πρέπει να σωπαίνει.

⚠️ Το Step 15a είχε γράψει ρητά «◀ σε παλιό κύκλο να αναφέρει σωστά αλλαγή που έχει ήδη συμβεί». Η **πρόθεση** ήταν ο κύκλος **ακριβώς πριν**· η υλοποίηση έπιανε κάθε μελλοντικό.

### Η διόρθωση
Νέα παράμετρος `until` (το `endExclusive` του κύκλου) και **ισότητα** αντί για `>`:

```ts
rates.find((rate) => rate.effectiveFrom.getTime() === until.getTime())
```

Το `effectiveFrom` είναι **πάντα** cycle-start instant (invariant του `schema.prisma`) και το `until` είναι η στιγμή που ανοίγει ο επόμενος κύκλος — άρα η ισότητα εκφράζει ακριβώς «αλλάζει η τιμή τη στιγμή που κλείνει αυτός ο κύκλος;».

⭐ Η **τιμολόγηση δεν άλλαξε καθόλου**: το `hourlyRate` προκύπτει ακόμα από το `at` (= `range.start`). Η νέα παράμετρος ορίζει μόνο τι επιτρέπεται να **ανακοινωθεί**. Γι' αυτό κανένα ποσό δεν κουνήθηκε πουθενά.

### Files added/changed
- `backend/src/users/users.service.ts` — `findEmployeeRateAt` (υπογραφή + λογική + doc)
- `backend/src/payroll/payroll.service.ts` — περνά `range.endExclusive`
- `backend/prisma/seed-demo.ts` — νέος employee **Ragnar Vilhjálmsson**
- **Tests**: `users.service.spec.ts` (+1 regression test), `payroll.service.spec.ts`, `test/payroll.e2e-spec.ts`

### Το seed
**Ragnar Vilhjálmsson** (`ragnar@demo.local`), τέταρτο άτομο στο `PEOPLE`, με **δύο** γραμμές `UserRate`: epoch/2400 και **25 Ιουλ/2700** (δόθηκε τον Ιούνιο, ισχύει από τον Ιούλιο).

- Flag `raise?` στο `DemoPerson`, όχι positional lookup — το μοτίβο που επιβάλλει ήδη το σχόλιο των γραμμών 55-63 για τα `boundaryShifts`/`openShift`.
- **Μέσα στο `PEOPLE`**, άρα το `buildShifts` δίνει βάρδιες και στους τρεις κύκλους αυτόματα.
- Pattern εξ ολοκλήρου σε ζώνη **DAY**: το νόημα είναι ότι *κινείται η τιμή*, όχι οι ζώνες.
- `hourlyRate: 2700` στη στήλη — το denormalised head, που το `schema.prisma` απαιτεί να συμφωνεί με τη νεότερη μη-μελλοντική γραμμή.
- Καμία ημερομηνία hardcoded: το instant βγαίνει από το `cycles[1].start`, άρα ακολουθεί το `cycleStartDay`.

### ⚠️ Δύο λάθος υποθέσεις που διορθώθηκαν στην πορεία
1. **«Ο τρέχων κύκλος δεν έχει βάρδιες»** — δεν ίσχυε. Με `cycleStartDay=25` ο τρέχων είναι `2026-08` (25 Αυγ→25 Σεπ) και όλο το `PEOPLE` έχει βάρδιες εκεί. Ο leaver έχει **μόνο** στον τρέχοντα **σκόπιμα**: το `payroll.service.ts:164` κόβει ανενεργό employee χωρίς ώρες, άρα βάρδιες σε παλιούς κύκλους θα τον **εξαφάνιζαν** από το overview.
2. **Πρώτη τοποθέτηση των γραμμών άφηνε κενό κύκλο ανάμεσα** — το ίδιο μήνυμα σε δύο συνεχόμενους κύκλους. Αυτό ήταν που οδήγησε στην ανακάλυψη του πραγματικού bug από πίσω.

### ⭐ Το e2e που έσπασε — και γιατί ήταν σωστό να σπάσει
Το `leaves an already-priced cycle byte-identical` **κωδικοποιούσε το bug**: ζητούσε τον `previous` κύκλο και περίμενε `pendingRate === RATE + 550`, ενώ η αύξηση ισχύει από τον **επόμενο** — δύο κύκλους μπροστά.

Ξαναγράφτηκε ώστε να συγκρίνει τα σώματα **ολόκληρα** (χωρίς `strip`, που πλέον δεν χρειάζεται: ούτε η ανακοίνωση επιτρέπεται να κουνηθεί) και να απαιτεί **σιωπή**. Προστέθηκε και η θετική πλευρά στο διπλανό test: ο **τρέχων** κύκλος *πρέπει* να ανακοινώνει, ο επόμενος όχι — η ανακοίνωση ανήκει σε **ακριβώς έναν** κύκλο.

### Verification που εκτελέστηκε
- ⭐ **Ζωντανά, μετά από `docker compose up -d --build backend`**:
  - `2026-06` → 2400 · pending **2700** ✅ (ο μόνος που ανακοινώνει)
  - `2026-07` → 2700 · pending **null** ✅
  - `2026-08` → 2700 · pending **null** ✅
  - Πριν τη διόρθωση θα εμφανιζόταν **και στους τρεις**.
- **Τιμή ανά ώρα**: 2400 / 2700 / 2700 — ο παλιός κύκλος κρατά την παλιά τιμή (Step 15 ορατό). ⚠️ Οι ώρες διαφέρουν ανά κύκλο, άρα το σκέτο `totalPay` δεν αποδεικνύει τίποτα· η διαίρεση ναι.
- **`GET /users`**: Ragnar 2700, **κανένα** badge — σωστό, η αύξηση ενεργοποιήθηκε ήδη. Το Team διαβάζει pending ως προς **τώρα** (`findPendingRates`, αμετάβλητο).
- Backend unit **242/242** (241 + 1 νέο regression test), e2e **127/127**.
- Frontend **235/235** σε 17 αρχεία.
- `tsc --noEmit` και `eslint` καθαρά.
- Seed δύο φορές → ίδιο αποτέλεσμα, κανένα unique-constraint violation.

⚠️ **Το δέκατο τέταρτο σφάλμα μέτρησης, ίδιο μοτίβο.** Πρώτο frontend run: `15 passed (15)` / `174 passed` με 2 errors σε **302s**. Καθαρό run: **17/17 σε 179s**. Δύο αρχεία δεν πρόλαβαν worker. **Ποτέ παράλληλα με άλλο βαρύ run.**

### ⚠️ Open, αμετάβλητα
1. **13b (Playwright)** — το τελευταίο ανοιχτό του build plan.
2. **§13 gap 7** και **gap 8**.

**Next step**: **13b (Frontend E2E, Playwright)**.

### Follow-up από `/review` (ίδια μέρα)
Το review βρήκε **4 ευρήματα**· διορθώθηκαν τα 3.

⭐ **Critical — τα context files περιέγραφαν τη συμπεριφορά που μόλις είχε αφαιρεθεί.** Δύο σημεία έλεγαν ρητά ότι «paging ◀ to an older cycle **correctly** reports a change that has since taken effect» — δηλαδή κατέγραφαν ως **σκόπιμο** ακριβώς το bug. Επιπλέον το `architecture.md` έγραφε την υπογραφή `findEmployeeRateAt(id, at)`, με δύο ορίσματα.

Γιατί μετράει περισσότερο από «ξεχασμένη τεκμηρίωση»: η γραμμή 770 βρίσκεται μέσα σε **κατάλογο invariants**, και το `AGENTS.md` ορίζει ότι οι invariants *«are never overridden by any prompt»*. Μια επόμενη συνεδρία που έβλεπε τον κώδικα να διαφωνεί με τον invariant θα ήταν **υποχρεωμένη να θεωρήσει λάθος τον κώδικα** — και να επαναφέρει το bug «διορθώνοντάς» το.

- `context/architecture.md` — υπογραφή στα 3 ορίσματα· αφαιρέθηκε η πρόταση που περιέγραφε την παλιά συμπεριφορά· νέο υπο-invariant: **ένας μόνο κύκλος ανακοινώνει αύξηση**, με ισότητα ως προς `endExclusive`, και ρητή αναφορά της σιωπηλής αστοχίας του gap 8.
- `context/swifttrack-phase1-final.md` §4 5g — η ίδια διόρθωση, με σήμανση «tightened in step 15b» ώστε να διαβάζεται ως **αλλαγή** και όχι σαν να ίσχυε ανέκαθεν.
- `README.md` — ο Ragnar στον demo roster· «three accounts that have one» → **four** (η καταμέτρηση έσπασε από το έκτο άτομο).
- `seed-demo.ts` — stale σχόλιο «three rows instead of one» → «a second row», μετά την αλλαγή σε μία αύξηση.

### ⚠️ Το λάθος πίσω από το critical, καταγεγραμμένο για να μην επαναληφθεί
Διάβασα στον tracker του 15a τη φράση «◀ σε παλιό κύκλο να αναφέρει σωστά αλλαγή που έχει ήδη συμβεί» και **υπέθεσα** ότι η πρόθεση ήταν «ο κύκλος ακριβώς πριν» και η υλοποίηση βγήκε πιο πλατιά. Η ερμηνεία μπορεί να είναι σωστή — αλλά ήταν **δική μου υπόθεση για γραπτή απόφαση κάποιου άλλου**, και βάσει αυτής άλλαξα κώδικα *και* ξαναέγραψα e2e test που πίναρε τη ρητά αντίθετη συμπεριφορά. Το σωστό ήταν να τεθεί ως ερώτημα: «ο invariant λέει Χ, εσύ αναφέρεις ότι το Χ είναι λάθος — αλλάζουμε τον invariant;». Το ότι το bug ήταν πραγματικό δεν αναιρεί ότι **η απόφαση να αλλάξει τεκμηριωμένη πρόθεση ανήκε στον χρήστη**.

### ⚠️ Important, συνειδητά ΜΗ διορθωμένο
**Η ισότητα κάνει το §13 gap 8 πιο σιωπηλό.** Το `effectiveFrom` είναι cycle-start instant *ως προς το `cycleStartDay` που ίσχυε όταν γράφτηκε*· μετακίνηση του ορίου το ρίχνει μεσοκύκλια, όπου η ισότητα δεν βρίσκει τίποτα.

Επαληθεύτηκε **εκτελώντας το**, όχι με υπόθεση:
```
effectiveFrom 2026-09-25, μεσοκύκλιο μετά τη μετακίνηση 25 → 15
OLD (> at)      pending: 3300   (λάθος κύκλος)
NEW (=== until) pending: null   (καμία αναφορά)
```
Θόρυβος → **σιωπή**. Η σιωπή είναι χειρότερη εδώ, γιατί το μήνυμα υπάρχει ακριβώς για να μη διαβαστεί η διαφορά ως **υποπληρωμή**.

Δύο μετριαστικοί παράγοντες, και οι δύο επαληθευμένοι στον κώδικα:
1. Το `resolveRateEffectiveFrom()` γράφει **πάντα** στην αρχή του επόμενου κύκλου = `endExclusive` του τρέχοντος, άρα η κανονική ροή δίνει ισότητα **by construction** — όχι κατά τύχη.
2. Το gap 8 είναι ήδη «accepted deliberately»: το `cycleStartDay` ρυθμίζεται μία φορά στο στήσιμο.

Δεν προκαλεί 500 (η epoch γραμμή το εγγυάται).

✅ **Η καταγραφή του §13 gap 8 διορθώθηκε** (follow-up commit). Έλεγε «slips a queued raise by one cycle» — περιέγραφε δηλαδή μόνο το μισό. Τώρα δηλώνει **δύο** συνέπειες: **(α)** πότε ενεργοποιείται η αύξηση (το αρχικό εύρημα, `<=` test) και **(β)** αν ανακοινώνεται καθόλου (νέο στο 15b, equality test → `null`). Γράφτηκε ρητά ότι πρόκειται για **επιδείνωση γνωστού gap** και όχι νέο bug, μαζί με το verified output. Η στήλη λύσης λέει πλέον ότι **μία** διόρθωση κλείνει και τα δύο μισά (ευθυγράμμιση των μελλοντικών γραμμών στο νέο όριο), και ⚠️ ότι **χαλάρωση της ισότητας πίσω σε «any future rate» ΔΕΝ είναι λύση** — αυτό είναι ακριβώς το bug του 15b, και θα αντάλλασσε μια σπάνια σιωπή με μόνιμο ψέμα σε κάθε παλιό κύκλο.

⚠️ **Το gap ίδιο παραμένει ανοιχτό** — μόνο η περιγραφή του διορθώθηκε.

### Verification μετά τα fixes
- Backend unit **242/242**, e2e **127/127**, `tsc` + `eslint` καθαρά.
- Frontend **δεν ξανατρέχτηκε**: τα fixes ήταν markdown + ένα σχόλιο backend· είχε επαληθευτεί **235/235** νωρίτερα στο ίδιο βήμα.

---

## Step 16 — Overtime: υπερωρίες πάνω από 173.33 ώρες/κύκλο (backend)
Status: ✅ Done
Date: 2026-09-11

Νέο feature, όχι διόρθωση: μέχρι τώρα η τιμή μιας ώρας εξαρτιόταν **μόνο** από το πότε δουλεύτηκε. Τώρα εξαρτάται και από **πόσες ώρες προηγήθηκαν μέσα στον κύκλο**.

### Απόφαση
| Ερώτημα | Απόφαση |
|---|---|
| Ποιες ώρες γίνονται overtime | **Χρονολογικά** — όσες δουλεύτηκαν αφού το σύνολο έφτασε τις 173.33 |
| Τιμή | **Σταθερός συντελεστής ×1.80 (+80%)** στο base, ανεξάρτητος ζώνης |
| Εμφάνιση | Πέμπτη ζώνη `OVERTIME`, σε **αμφότερους** τους πίνακες |

### Files added/changed
- `src/payroll/rate-zones.util.ts` — `PayZone.OVERTIME`· πέμπτη εγγραφή στο `PAY_ZONES` (factor 180)· `OVERTIME_THRESHOLD_CENTIHOURS = 17_333`· `startMs` στο `DayZoneSegment`· το κατώφλι μέσα στον `buildDayZoneHours`.
- `src/payroll/payroll.service.ts` — μόνο το `toDayDto` (πέμπτο κλειδί). Το κατώφλι **δεν** ακουμπά τον service.
- `src/payroll/dto/payroll-response.dto.ts` — `DayZoneHoursDto.OVERTIME`· «four» → «five».
- `test/helpers/types.ts` — ο τύπος `PayrollZone`/`PayrollDay` των e2e· **το είχα ξεχάσει** και το έπιασε ο `tsc`, όχι τα tests.
- Specs: `rate-zones.util.spec.ts` (+7 νέα), `payroll.service.spec.ts` (+1), `payroll.e2e-spec.ts` (+1).
- Context: `swifttrack-phase1-final.md` §7, `architecture.md` (Payroll Pattern + 2 νέοι invariants).

### ⭐ Η σχεδιαστική απόφαση που κόστισε τον περισσότερο χρόνο
Δύο υλοποιήσεις εξετάστηκαν και **μετρήθηκαν** (3.000 τυχαίοι κύκλοι, πραγματικός κώδικας, σημείο αναφοράς προσομοίωση λεπτό-προς-λεπτό):

| | Μέγιστο σφάλμα | Σε ISK (base 2450) |
|---|---|---|
| **Β — κατώφλι στα ταξινομημένα segments (επιλέχθηκε)** | **0.04 h** | ~176 |
| Α — κατώφλι στα συγκεντρωτικά κελιά, σειρά NIGHT→DAY→EVENING | 0.07 h | ~309 |
| *Baseline: η εφαρμογή πριν το feature* | *0.073 h* | *~323* |

Επίδοση: **καμία μετρήσιμη διαφορά** (200 εργαζόμενοι / 12.468 βάρδιες: ~110ms, όσο και πριν).

Επιλέχθηκε το Β **όχι για την ακρίβεια** — η διαφορά είναι μικρότερη από το σφάλμα που ήδη υπήρχε — **αλλά γιατί δεν μπορεί να χαλάσει σιωπηλά**. Το Α είναι σωστό μόνο επειδή οι ζώνες τυχαίνει να ορίζονται από σταθερά όρια ώρας· υπόθεση πουθενά γραμμένη ως κανόνας.

⚠️ **Η πρώτη εκδοχή του πλάνου ήταν λάθος κατά 8 ώρες/κύκλο** — χρησιμοποιούσε τη σειρά *εμφάνισης* των ζωνών (DAY πρώτο) αντί της χρονολογικής (NIGHT πρώτο). Το εντόπισε ο χρήστης ρωτώντας «είσαι σίγουρος;» πριν γραφτεί κώδικας. Καταγράφεται γιατί είναι ακριβώς η αστοχία που ο invariant στο `architecture.md` υπάρχει για να αποτρέψει.

### Notes για το επόμενο βήμα
- Το κατώφλι ζει **μέσα** στον `buildDayZoneHours`, όχι στον service. Σκόπιμα: ατομική σελίδα και overview περνούν και οι δύο από εκεί, άρα δεν υπάρχει δεύτερος δρόμος να ξεχαστεί ο ένας.
- Ο `resolveZone()` **δεν** επιστρέφει ποτέ `OVERTIME`. Είναι το πρώτο σημείο όπου `PayZone` ≠ «ζώνη του ρολογιού».
- Το worked example (42.62 h) **αμετάβλητο** — καμία παλινδρόμηση για όποιον δεν κάνει υπερωρίες.

### Verification που εκτελέστηκε
- Backend unit **250/250** (242 → +8), e2e **128/128** (127 → +1), `tsc --noEmit` + `eslint` καθαρά.
- ⭐ **Ζωντανά** (`docker compose up -d --build backend`, νέος εργαζόμενος @2450, 20 βάρδιες × 9h = 180h στον `2026-07`):
  - `DAY 173.33 @2450 = 424.659` · `OVERTIME 6.67 @4410 = 29.415` · total **454.074**
  - πέντε ζώνες, `OVERTIME` τελευταίο· κανονικές ζώνες **ακριβώς 173.33**
  - `Σ zones[].pay === totalPay` ✅ · `pay === round(hours × rate)` σε κάθε γραμμή ✅
  - ημέρα του ορίου (21/08): `DAY 2.33 + OVERTIME 6.67 = 9.00` — η γραμμή αθροίζει ακόμα τις ώρες που δουλεύτηκαν ✅
  - `/payroll/overview` δίνει **το ίδιο** totalPay ✅
  - καθαρισμός: βάρδιες διαγράφηκαν, λογαριασμός απενεργοποιήθηκε, `totalCost` ανεπηρέαστο.

### ⚠️ Ανοιχτά
1. **Step 16a (frontend)** — δεν ξεκίνησε· το gate 8b/AGENTS.md απαιτεί πρώτα επαληθευμένο backend. Αναμένεται ελάχιστη δουλειά: `PayrollSummary` και `PayrollDayTable` παράγουν ήδη γραμμές/στήλες από το `zones[]`. Μένουν ο τύπος στο `api/payroll.ts`, το `SHORT_ZONE_LABELS.OVERTIME`, fixtures και οπτικός έλεγχος 7 στηλών σε στενή οθόνη.
2. **13b (Playwright)** και **§13 gaps 7, 8** — αμετάβλητα από πριν.

**Next step**: **16a (Overtime, frontend)**.

### Follow-up από `/review` (ίδια μέρα)
Το review βρήκε **4 ευρήματα**· διορθώθηκαν και τα 4.

⭐ **Critical — η πιο κρίσιμη γραμμή του feature δεν ελεγχόταν.** Το mutation testing (αφαίρεση του `.sort((a,b)=>a.startMs-b.startMs)`) άφησε **και τα 71 tests του payroll πράσινα**. Αποδείχθηκε ότι η μετάλλαξη αλλάζει πραγματικά χρήματα:

```
κανονικά:  DAY 171.00  NIGHT 0.33  OVERTIME 5.67
ανάποδα:   DAY 165.33  NIGHT 6.00  OVERTIME 5.67   ← 5.67 h σε ζώνη άλλης τιμής
```

**Αιτία**: το test «does not depend on the order» χρησιμοποιούσε 22 **πανομοιότυπες** 9ωρες DAY βάρδιες — μια λίστα συμμετρική ως προς την αντιστροφή, άρα περνούσε είτε υπήρχε ταξινόμηση είτε όχι.

**Διόρθωση**: δεδομένα **ασύμμετρα** ως προς την αντιστροφή — 19 βάρδιες + μία νυχτερινή που περνά τα μεσάνυχτα, όπου το EVENING μισό δουλεύεται *πριν* το NIGHT μισό της ίδιας βάρδιας. Προστέθηκαν και pinned τιμές, ώστε η ισότητα να μην ικανοποιείται με **δύο** λάθος πλευρές.

⚠️ **Επαληθεύτηκε εκτελώντας το**, όχι με υπόθεση: με αφαιρεμένη την ταξινόμηση το test **κοκκινίζει** (1 failed / 46 passed)· με την ταξινόμηση πράσινο (47/47). Αυτό είναι η απόδειξη ότι το test δεν είναι κενό.

⚠️ **Η προηγούμενη δήλωσή μου ότι «τα tests κλειδώνουν την ταξινόμηση» ήταν λάθος.** Είχε γραφτεί χωρίς να ελεγχθεί.

**Τα άλλα 3:**
- **Important** — `payroll.controller.ts` Swagger έλεγε ακόμα «the four zones» ενώ το API επιστρέφει πέντε. Είναι το δημόσιο συμβόλαιο· ίδιο μοτίβο απόκλισης doc/κώδικα με το critical του 15b.
- **Minor** — `rate-zones.util.ts` σχόλιο στο `DayZoneHours`: «All four zones always present» → five.
- **Minor** — το e2e test όριζε τοπικό `zone()` που **επισκίαζε** τον helper του αρχείου· ο τοπικός χρησιμοποιούσε `!` και θα εμφάνιζε μια ζώνη που λείπει ως `TypeError` σε άσχετη ιδιότητα — ακριβώς ό,τι ο πρωτότυπος helper υπάρχει για να αποτρέψει. Αφαιρέθηκε.

### Επίσης επαληθεύτηκε στο review (καμία ενέργεια)
- **1.500 τυχαίοι κύκλοι, όλοι με υπερωρίες**: 0 παραβιάσεις του «κάθε γραμμή αθροίζει στο σύνολό της» και του «τα σύνολα ζωνών είναι ακριβή αθροίσματα κελιών».
- Ανοιχτές βάρδιες **δεν** καταναλώνουν χώρο κατωφλίου ✓· το κατώφλι είναι ακέραιο σε ms (623.988.000) ✓.
- ⚠️ **Γνωστό, δεν είναι bug**: με πολλές βάρδιες περίεργου μήκους οι τέσσερις κανονικές ζώνες μπορεί να δώσουν 173.30 αντί 173.33 (−3 centihours). Ακολουθεί την **προϋπάρχουσα** στρογγυλοποίηση ανά κελί (−3.56 αστρογγύλευτο), όχι το κατώφλι — κανένα νέο σημείο στρογγυλοποίησης.

### Verification μετά τα fixes
- Backend unit **250/250**, e2e **128/128**, `tsc --noEmit` + `eslint` καθαρά.
- Mutation test επαναλήφθηκε και **κοκκινίζει** σωστά.

---

## Step 16a — Overtime (frontend)
Status: ✅ Done
Date: 2026-09-11

Το βήμα που ο σχεδιασμός των ζωνών είχε φτιαχτεί για να είναι φθηνό — **και όντως ήταν**.

### ⭐ Κανένα από τα δύο payroll components δεν άλλαξε
Το σχόλιο στο `PayrollSummary.tsx:26-28` έλεγε ότι «μια πέμπτη ζώνη εμφανίζεται εδώ χωρίς καμία αλλαγή σε αυτό το αρχείο». **Επαληθεύτηκε στην πράξη**: και τα δύο παράγουν γραμμές/στήλες από το `zones[]`, οπότε αρκούσαν δύο γραμμές αλλού.

### Files changed
- `src/api/payroll.ts` — `PayZone` + `"OVERTIME"`. Το `DayZoneHours` είναι `Record<PayZone, number>`, άρα τα κελιά ακολούθησαν μόνα τους.
- `src/lib/messages.ts` — `SHORT_ZONE_LABELS.OVERTIME = "Overtime"` (επικεφαλίδα χωρίς ποσοστό, §8a).
- Specs: `PayrollDayTable.spec.tsx`, `PayrollSummary.spec.tsx`, `PayrollPage.spec.tsx` — fixtures + 2 νέα tests.

### ⚠️ Τέσσερα tests έσπασαν σε hardcoded δείκτες στηλών
`cells[5]`, `cells[4]`, `toHaveLength(5)` — δηλώσεις **θέσης** που μετακίνησε η νέα στήλη. Ξαναγράφτηκαν ώστε να παράγονται από τα δεδομένα (`cells[cells.length - 1]`, `ZONES.length + 1`), άρα η επόμενη ζώνη δεν θα τα ξανασπάσει.

Αυτό είναι το ίδιο μοτίβο με το critical του Step 16: **ένα test που κωδικοποιεί το σχήμα των δεδομένων αντί για την ιδιότητα** σπάει (ή, χειρότερα, περνά) για λάθος λόγο.

### Νέα tests
- `PayrollDayTable` — η ημέρα του ορίου: `DAY 2.33 + OVERTIME 6.67`, σύνολο γραμμής **9.00**. Ο εργαζόμενος βλέπει ότι η γραμμή αθροίζει ακόμα τις ώρες που δούλεψε.
- `PayrollSummary` — η γραμμή «Overtime +80%» με `6.67 / 4,410.00 / 29,415`.

### Verification που εκτελέστηκε
- Frontend **237/237** σε **17 αρχεία** (235 → +2), `tsc -b` + `eslint` καθαρά.
- ⚠️ **Το δέκατο πέμπτο σφάλμα μέτρησης, ίδιο μοτίβο.** Πρώτο run: `4 failed | 213 passed` **με worker timeout** στο `SettingsPage.spec` σε 201s. Καθαρό run: **237/237 σε 121s**. Οι 4 αποτυχίες ήταν πραγματικές (οι δείκτες στηλών)· ο timeout **όχι**.
- ⭐ **Ζωντανά** (`docker compose up -d --build frontend`, εργαζόμενος 44 @2450, 179h στον `2026-07`):
  - `DAY 171.00` · `EVENING 2.00` · `NIGHT 0.33` · `OVERTIME 5.67 @4410 = 25.005` · total **451.644**
  - Τα δεδομένα αναπαράγουν **ακριβώς** το σενάριο NIGHT/EVENING του mutation test — η νυχτερινή βάρδια που περνά τα μεσάνυχτα.
  - Επικεφαλίδες: `Date | Day | Evening | Night | Weekend | Overtime | Total` — **7 στήλες**.
  - Γραμμές αποδοχών: `Day / Evening +33% / Night +45% / Weekend +45% / Overtime +80%`.
  - SPA route `/payroll/44` → 200.

### Responsive
Ο ημερήσιος πίνακας πήγε **6 → 7 στήλες**. Και οι δύο πίνακες είναι ήδη σε `overflow-x-auto`, και το shadcn `Table` έχει δικό του· τα κελιά είναι `whitespace-nowrap`. Άρα το επιπλέον πλάτος κυλά **μέσα στον πίνακα**, όχι στη σελίδα.

⚠️ **Δεν έγινε έλεγχος σε πραγματικό browser σε πλάτος κινητού.** Το Playwright δεν είναι εγκατεστημένο (step 13b ανοιχτό) και δεν εγκαταστάθηκε εξάρτηση γι' αυτό. Η ανάλυση είναι από τις κλάσεις, όχι από μέτρηση — **ανοιχτό για το 13b**.

### ⚠️ Ανοιχτά
1. **Οπτικός έλεγχος σε κινητό** — βλ. παραπάνω· φυσικό μέρος του 13b.
2. **13b (Playwright)**, **§13 gaps 7, 8** — αμετάβλητα.
3. Ο demo εργαζόμενος `overtime.demo@demo.local` (id 44) **παρέμεινε** στη βάση με 20 βάρδιες, σκόπιμα: είναι το μόνο προφίλ που δείχνει υπερωρίες στο UI. Να αφαιρεθεί ή να μπει στο `seed-demo.ts` αν χρειαστεί μόνιμα.

**Next step**: **13b (Frontend E2E, Playwright)** — το τελευταίο ανοιχτό του build plan.

### Follow-up: έλεγχος README + Swagger (ίδια μέρα, μετά το 16a)
Ο χρήστης ζήτησε να επαληθευτεί ότι η τεκμηρίωση προς τα έξω είναι ενημερωμένη. Βρέθηκαν **3 ζητήματα**, όλα διορθωμένα.

⭐ **README — το feature δεν υπήρχε πουθενά.** Έλεγε «Hours are split across **four** rate zones» και ο πίνακας ζωνών είχε τέσσερις γραμμές. Είναι το πρώτο πράγμα που διαβάζει ο εξεταστής, και το overtime ήταν αόρατο.
- Διορθώθηκε η πρόταση + **πέμπτη γραμμή** στον πίνακα: `| Overtime | Every hour past 173.33 in the cycle | +80% |`.

⚠️ **`payroll-response.dto.ts:56` — «do not add the four cells yourself»** ενώ τα κελιά είναι πέντε. Το ίδιο μοτίβο doc/κώδικα με το important του review· διορθώθηκε.

⭐ **Το demo seed ΗΔΗ δείχνει υπερωρίες — κανείς δεν το ήξερε.** Επαληθεύτηκε ζωντανά στον **ολοκληρωμένο** κύκλο `2026-07`:
```
anna@demo.local   175h | OT  1.67h →  7.365 ISK
bjorn@demo.local  184h | OT 10.67h → 53.777 ISK
```
**Δεν χρειάστηκε νέο demo πρόσωπο.** Το README όμως δεν το έλεγε, και υπήρχε πραγματική παγίδα: ο **τρέχων** κύκλος δείχνει 0 overtime για όλους (οι βάρδιες σταματούν στο «χθες», άρα δεν έχει φτάσει τις 173.33). Ένας εξεταστής που άνοιγε το Payroll θα συμπέραινε ότι το feature δεν δουλεύει.
- Ο Björn απέκτησε σημείωση στο roster· προστέθηκε ⚠️ ότι το overtime φαίνεται **μόνο σε ολοκληρωμένο κύκλο — πάτα ◀ μία φορά**.

### Επίσης ελέγχθηκαν (καμία ενέργεια)
- **Ζωντανό Swagger** μετά από rebuild: `zone` enum = `["DAY","EVENING","NIGHT","WEEKEND","OVERTIME"]`, `DayZoneHoursDto` πέντε κλειδιά, `zones[]` «Always five entries» ✅
- Οι υπόλοιπες αναφορές σε «four» στον κώδικα είναι **σωστές** — αναφέρονται σκόπιμα στις τέσσερις ζώνες *ρολογιού*, σε αντιδιαστολή με το OVERTIME.
- `main.ts` Swagger title/description: γενικά, παραμένουν σωστά.
- Δεν υπάρχουν άλλα user-facing docs (μόνο README· AGENTS/CLAUDE είναι για τον agent).

**Επαλήθευση**: `tsc --noEmit` + `eslint` καθαρά· backend rebuilt και το ζωντανό `/api-json` επιβεβαιώθηκε.

---

## Step 17 — Έξι μικροδιορθώσεις (δύο ήταν ήδη υλοποιημένες, μία αναιρέθηκε)
Status: ✅ Done
Date: 2026-09-11

⚠️ **Αρχικά γράφτηκε ως «Step 16», που ήταν ήδη πιασμένο** από το Overtime backend. Δύο τμήματα με τον ίδιο αριθμό κάνουν τον tracker αναξιόπιστο ως χρονολόγιο — διορθώθηκε σε 17.

Ο χρήστης ανέφερε έξι σημεία. Η διερεύνηση άλλαξε ουσιαστικά τρία από αυτά πριν γραφτεί κώδικας, και ένα ακόμη **αναιρέθηκε μετά από δικό του έλεγχο**.

### Τι δεν χρειάστηκε καθόλου κώδικα

**#2 (μήνυμα ισλανδικής ώρας)** — υπήρχε ήδη: `TimezoneNotice`, mounted global στο `App.tsx`, με το δυναμικό `NOTICES.timezone(zone, difference)`. Δεν φαινόταν επειδή **κρύβεται σκόπιμα όταν το offset είναι 0** — δηλαδή σε μηχάνημα UTC ή σε Docker. Ο χρήστης επιβεβαίωσε ότι αυτή είναι η επιθυμητή συμπεριφορά.

**#6 (toasts)** — `sonner` εγκατεστημένο, `<Toaster />` mounted, σε χρήση σε TeamPage/ShiftHistoryPage.

⚠️ Και τα δύο καταγράφονται εδώ ώστε να μην «ξαναφτιαχτούν» σε επόμενη συνεδρία.

### #1 — Προσυμπλήρωση Add Shift
`ShiftForm.defaultValues` δίνει `toDatetimeLocal(nowIsoUtc())` στο start σε add mode.

⚠️ **Το end μένει κενό σκόπιμα**: το `handleStartBlur` το γεμίζει από το start *επειδή* είναι κενό, άρα προσυμπλήρωση και των δύο θα άφηνε μηδενική βάρδια ένα Save μακριά.

### #3 — Το αρχικό αίτημα δεν γινόταν· το πραγματικό bug ήταν αλλού
Ζητήθηκε «το ημερολόγιο να ανοίγει προς τα πάνω». Το popup του `datetime-local` είναι **browser chrome** — η θέση του δεν ελέγχεται με CSS, και η αναδιάταξη πεδίων είναι θεατρινισμός (ο picker αγκιστρώνεται στο focused input, άρα απλώς καλύπτει κάτι άλλο).

Βρέθηκε **δεύτερο**, γνήσιο bug: `DialogContent`/`AlertDialogContent` είναι κεντραρισμένα με `-translate-y-1/2` **χωρίς `max-h` και χωρίς `overflow`**, οπότε περιεχόμενο ψηλότερο από το viewport σπρώχνει την κορυφή εκτός οθόνης, όπου το `position: fixed` την αφήνει **απρόσιτη**. Αφορά **κάθε** dialog.

### ✅ #3 — λύθηκε στη **δεύτερη** προσπάθεια· η πρώτη ήταν σπασμένη και αναιρέθηκε

⚠️ **Αυτή η ενότητα γράφτηκε τρεις φορές μέσα στην ίδια συνεδρία** (μπήκε → αναιρέθηκε → ξαναμπήκε διορθωμένο), και μια ενδιάμεση εκδοχή της έμεινε να δηλώνει «καμία διόρθωση δεν στέκει στο δέντρο» ενώ η διόρθωση ήταν ήδη μέσα. Το `/review` το έπιασε ως **critical**: είναι το ίδιο μοτίβο με το critical του 15b — τεκμηρίωση που περιγράφει συμπεριφορά η οποία έχει αλλάξει, σε αρχείο που το `AGENTS.md` κάνει υποχρεωτική ανάγνωση. Μια επόμενη συνεδρία θα ξαναέγραφε το D ή θα έσβηνε το υπάρχον ως νεκρό κώδικα.

**Τι στέκει τώρα στον κώδικα** (και τα τρία επαληθευμένα με μέτρηση):

- `dialog.tsx` → `max-h-[calc(100dvh-2rem)]` + **`grid-rows-[minmax(0,1fr)]`** + `overflow-hidden` στο popup, και νέο `DialogBody` slot που σκρολάρει. Header, footer και close button μένουν **εκτός** του scroller, άρα παραμένουν καρφωμένα χωρίς sticky positioning.
- `alert-dialog.tsx` → μόνο `max-h` + `overflow-y-auto` στο **ίδιο** το popup. Έχει **μεταβλητό** αριθμό παιδιών (τίτλος, πρόταση, προαιρετικό σφάλμα, footer), οπότε σταθερό `grid-rows` θα ήταν λάθος. Ίδιο σχήμα με `dropdown-menu`/`select`.
- `ShiftForm` + `EmployeeForm` → `min-h-0` στο `<form>` και τα πεδία μέσα σε `<DialogBody>`.

⭐ **Το `grid-rows-[minmax(0,1fr)]` είναι ο λόγος που η δεύτερη προσπάθεια δούλεψε.** Χωρίς αυτό, η implicit grid row έχει `min-height: auto` και αρνείται να συρρικνωθεί — μετρημένα, το `DialogBody` ανέφερε `overflows: false` και το περιεχόμενο απλώς ξεχείλιζε έξω από το `overflow-hidden`: **κομμένο χωρίς scrollbar**, χειρότερο από το αρχικό bug.

⚠️ **Το αρχικό αίτημα (picker προς τα πάνω) παραμένει αδύνατο** — browser chrome, εκτός stacking context. Αυτό που λύθηκε είναι ότι **τα κουμπιά μένουν πάντα προσιτά**.

⚠️ Επαληθεύτηκε ότι το Base UI v1.6.0 δεν προσφέρει τίποτα εδώ (`DialogPopupProps` = μόνο `initialFocus`/`finalFocus`). Το `max-h-(--available-height)` των dropdown/select **δεν ισχύει** για dialog — αυτά έχουν Positioner, ο Dialog όχι.

### ⚠️ Τα λάθη της **πρώτης** προσπάθειας — ισχύουν ακόμα ως μαθήματα

1. **Έγραψα σχόλιο μέσα στον κώδικα που έλεγε «κλασική flex παγίδα — επαλήθευση σε browser, όχι εμπιστοσύνη», και μετά έκανα commit χωρίς να την επαληθεύσω.** Το πλάνο έλεγε το ίδιο. Δεν υπάρχει browser driver εγκατεστημένος και το jsdom δεν κάνει layout, άρα η επαλήθευση ήταν **αδύνατη** με τα διαθέσιμα εργαλεία — αυτό έπρεπε να σταματήσει το commit, όχι να συνοδεύσει ένα σχόλιο.

2. ⭐ **Η προσέγγιση ήταν όντως λάθος — επιβεβαιώθηκε με μέτρηση, όχι με υπόθεση.** Το `DialogContent` είναι `grid` χωρίς ρητά `grid-rows`, άρα η φόρμα κάθεται σε **implicit auto-sized γραμμή** με `min-height: auto`, που **αρνείται** να συρρικνωθεί. Σε συνδυασμό με `overflow-hidden` το αποτέλεσμα ήταν **περιεχόμενο κομμένο χωρίς scrollbar** — χειρότερο από το αρχικό bug, γιατί το overflow γίνεται **αόρατο** αντί απλώς απρόσιτο. Το headless Chrome το έδειξε ως `overflows: false` σε 497/397/317px. Η λύση ήταν το `grid-rows-[minmax(0,1fr)]`, όχι η εγκατάλειψη της προσέγγισης.

3. **Δήλωσα «ολοκληρώθηκε» χωρίς να ξαναχτίσω το frontend container.** Έκανα `up -d --build backend` και επαλήθευσα το #5 μέσω API — που δούλεψε, γιατί είναι backend. Τα #1 και #4 **δεν έτρεξαν ποτέ**: ο χρήστης τα ανέφερε ως «δεν φτιάχτηκε» και είχε δίκιο ως προς ό,τι έβλεπε. Επιβεβαιώθηκε με `grep` στο σερβιρισμένο bundle: `dialog-body` → **0 εμφανίσεις**, assets από **01:27** (13 ώρες πριν τις αλλαγές).

### #3b ⭐ — Χωριστά πεδία date + time αντί για `datetime-local`

Η δεύτερη μισή απάντηση στο #3, και **η μεγαλύτερη σχεδιαστική αλλαγή της συνεδρίας**. Το D κρατά τα κουμπιά προσιτά όταν το *dialog* είναι ψηλό· αυτό αντιμετωπίζει το ότι ο *picker* τα σκεπάζει.

**Η μέτρηση που την επέβαλε** (headless Chrome, 768/900/1080px):

| | |
|---|---|
| Απόσταση End field → Save | **152px** |
| Ύψος popup `datetime-local` | **~320px** |
| Σκέπαζε το Save | σε **κάθε** ύψος οθόνης |

⚠️ Και **δεν αυτοδιορθώνεται**: ο Chrome γυρίζει το popup προς τα πάνω **μόνο** όταν δεν χωράει κάτω — και κάτω πάντα χωρούσε (έφτανε στα 572px σε viewport 665px). Δοκιμάστηκε και top-anchoring του dialog: `endCoversSave=true` και πάλι, γιατί η απόσταση input→Save είναι σταθερή **μέσα** στο dialog.

**Η λύση**: τέσσερα inputs (`date` + `time` σε κάθε άκρο). Το `time` popup είναι σύντομη λίστα ωρών αντί για ημερολόγιο, και το dialog μίκρυνε **513 → 432px**.

- Νέοι helpers στο `lib/datetime.ts` — **υποχρεωτικά εκεί**, ο invariant απαιτεί μία πόρτα για ημερομηνίες: `toDateAndTime(iso)` → `[date, time]` και `fromDateAndTime(d, t)` → `datetime-local` ή `""`.
- ⚠️ Το `fromDateAndTime` επιστρέφει `""` όταν **οποιοδήποτε** μισό λείπει, αντί να μαντέψει μεσάνυχτα. Μια ημερομηνία χωρίς ώρα δεν είναι στιγμή· το `""` πέφτει στον κανόνα `required` του schema, που είναι το ένα σημείο που αποφασίζει τι σημαίνει ημιτελές πεδίο.
- Το schema έχει **τέσσερα** πεδία· τα cross-field μηνύματα δένονται στο **date** μισό — εκεί διορθώνεται μια νυχτερινή βάρδια (`20:00 → 03:00` φτιάχνεται μετακινώντας την *ημερομηνία*, όχι το ρολόι).
- Το `handleStartDateBlur` αντιγράφει **μόνο την ημερομηνία**. Η παλιά έκδοση έσερνε ολόκληρη στιγμή, άρα και το ρολόι· τώρα το ρολόι μένει κενό και περιμένει την τιμή που ο χρήστης πάντα πρέπει να δώσει.
- Νέα `LABELS.dateOf`/`clockOf` → `aria-label` ανά input. Το ορατό `FieldLabel` ονομάζει την **ομάδα**, οπότε χωρίς αυτά δύο inputs θα μοιράζονταν ένα όνομα.

⭐ **Δεύτερο όφελος**: η ημερομηνία λήξης γίνεται **ορατό πεδίο** αντί για segment θαμμένο μέσα σε σύνθετο control — ακριβώς αυτό που μια νυχτερινή βάρδια χρειάζεται να προσέξει ο χρήστης.

⚠️ **Η εμφάνιση των native inputs ακολουθεί τη γλώσσα του BROWSER, όχι του app.** Μετρήθηκε: το `lang="en-GB"` στη σελίδα **δεν** επηρεάζει· ο Chrome δείχνει `08/04/2026` και `04:00 PM`. Άρα η ίδια βάρδια μπορεί να διαβάζεται `04 Aug 2026, 16:00` στη λίστα (που περνά από `en-GB` του `datetime.ts`) και `08/04/2026 04:00 PM` στη φόρμα. **Η αποθηκευμένη τιμή είναι πάντα σωστή** — το `.value` είναι `2026-08-04`/`16:00` ανεξαρτήτως locale, επαληθευμένο.

### #5 ⭐ — `hourlyRate`: αποκατάσταση, όχι επανασχεδιασμός
**Τρία ανεξάρτητα σημεία τεκμηρίωσης το όριζαν ήδη**: `architecture.md` (μέσα στους **Invariants**), `swifttrack-phase1-final.md:39`, και `user-response.dto.ts:49` — όλα λένε «what they are paid **now**». Ο κώδικας διαφωνούσε και με τα τρία.

⚠️ Αυτό είναι το **αντίστροφο** του λάθους του 15b: εκεί άλλαξα τεκμηριωμένη πρόθεση βάσει δικής μου υπόθεσης· εδώ ο κώδικας ευθυγραμμίστηκε με invariant που ήδη υπήρχε γραπτός.

**Το bug**: `updateEmployee` γράφει τη νέα τιμή στο `User.hourlyRate` **αμέσως**, ενώ η γραμμή `UserRate` παίρνει μελλοντικό `effectiveFrom`. Τα `toResponseDto`/`toProfileDto` επέστρεφαν τη στήλη → `/users`, `/users/me` **και το login** ανέφεραν τιμή που κανείς δεν πληρώνεται.

**Δεύτερο θύμα, εκτός αρχικής αναφοράς**: το profile/login. Ο employee έβλεπε **μεγαλύτερη** τιμή στο προφίλ του απ' ό,τι στο payroll — το σύμπτωμα που η προδιαγραφή §5g κατονομάζει ρητά.

- Νέο `findRatesNow(userIds)` → `{current, pending}` από **ένα** query, με **ένα** `now` διαβασμένο πριν τον βρόχο. Τα `findPendingRates`/`findPendingRate` διαγράφηκαν.
- `toResponseDto` παίρνει το bundle — **υποχρεωτική παράμετρος χωρίς default**, ώστε ο compiler να απαριθμήσει και τα 7 call sites.
- Νέο `toProfileFor()` με ADMIN short-circuit σε **ένα** σημείο· `auth.service.ts` άλλαξε κατά μία λέξη.

⚠️ **Η παγίδα του `updateEmployee`**: στη **δεύτερη** αύξηση του ίδιου κύκλου το `employee.hourlyRate` **δεν** είναι η τιμή σε ισχύ (κρατά ήδη την πρώτη αύξηση). Άρα χρειάζεται ανάγνωση — **πριν** το transaction, ασφαλώς, γιατί το write αγγίζει μόνο μελλοντική γραμμή.

⚠️ **Το `rateChanged` συνεχίζει να συγκρίνει με τη στήλη**, και αυτό είναι η μόνη δουλειά που της απέμεινε: με pending 3200 και τον admin να ξαναϋποβάλλει 3200, σύγκριση με την εν ισχύ θα έκανε άσκοπο upsert σε **κάθε** save.

### #5 frontend — υποχρεωτικό συνοδευτικό, όχι προαιρετικό
⚠️ Το `EmployeeForm` στέλνει `hourlyRate` **πάντα**, ακόμα και σε σκέτη μετονομασία. Μετά το backend fix η φόρμα θα έδειχνε την εν ισχύ τιμή και ένα Save θα **έσβηνε σιωπηλά** την αύξηση. Prefill → `pendingRate ?? hourlyRate`. **Ίδιο commit** με το backend: ενδιάμεσα υπάρχει παράθυρο data loss.

Οι δύο οθόνες απαντούν σε διαφορετικές ερωτήσεις: η **στήλη** «τι πληρώνεται;» → 2450 + badge· η **φόρμα** «τι είναι ρυθμισμένο;» → 3200.

### #4 — Team tooltips + επιβεβαίωση σε όλα
`npx shadcn@latest add tooltip` (Base UI, μηδέν Radix). Νέο `RowAction` helper — tooltip + `aria-label` **και τα δύο** (το tooltip δεν είναι accessible name). Δύο νέα dialogs (`ReactivateEmployeeDialog`, `NewCodeDialog`) αντιγράφοντας ακριβώς το σχήμα: **αποτυχία κρατά ανοιχτό** με τον λόγο μέσα.

⚠️ **Το shadcn CLI εγκατέστησε άσχετο npm package `cn`** και έγραψε `import { cn } from "cn"`. Αφαιρέθηκε· το import διορθώθηκε σε `@/lib/utils`. **Να ελέγχεται το `package.json` μετά από κάθε `shadcn add`.**

⚠️ **Duplicate key σε object literal — σιωπηλό, όχι compile error.** Το `newCodeTitle` υπήρχε ήδη (τιτλοφορεί το dialog που δείχνει τον κωδικό *μετά* το write) και το επανακαθόρισα για την επιβεβαίωση *πριν*. Μετονομάστηκε σε `confirmNewCodeTitle`/`confirmNewCodeBody`. Το μόνο σύμπτωμα ήταν λάθος επικεφαλίδα σε dialog που δεν είχε αγγιχτεί.

### ⚠️ Τέσσερις παγίδες εργαλείων, καταγεγραμμένες
1. **`npx prettier --write` ΔΕΝ είναι το formatter του project** — πρόσθεσε semicolons παντού. Το project χρησιμοποιεί `eslint --fix` με δική του διαμόρφωση. Χρειάστηκε `git checkout` και επανάληψη των αλλαγών.
2. **Σκληρές ημερομηνίες σε tests** — το `NEXT_CYCLE_START = '2026-08-25'` ήταν στο **παρελθόν** μέχρι σήμερα. Τα παλιά tests πέρναγαν επειδή τα mocks δεν εφάρμοζαν ποτέ φίλτρο· το `findRatesNow` κάνει partition στη μνήμη, άρα η ημερομηνία μετράει. Οι σταθερές είναι πλέον **σχετικές με το `Date.now()`**.
3. 🔴 **`curl` από Git Bash σε Windows κατέστρεψε δεδομένα.** Ένα `curl -d '{"name":"Anna Jónsdóttir"}'` έστειλε το `ó` σε **Windows-1252**· ο server το διάβασε ως UTF-8, βρήκε άκυρα bytes και αποθήκευσε `U+FFFD` — μόνιμα, στη βάση. ⚠️ **Το είχα τυπώσει στην ίδια μου την έξοδο** (`Anna J�nsd�ttir: profile rate 2450`) και κοίταξα μόνο το «2450»· το βρήκε ο χρήστης. Η **εφαρμογή ήταν αθώα** — τα άλλα έξι ισλανδικά ονόματα έμειναν σωστά, άρα το UTF-8 δουλεύει end-to-end. Διορθώθηκε με Node (`http` + ρητό `Buffer.from(…,'utf8')`, όνομα χτισμένο από code points). **Κανόνας: μη στέλνεις non-ASCII μέσω `curl` από Git Bash — χρησιμοποίησε Node.**
4. 🔴 **Δήλωσα «ολοκληρώθηκε» χωρίς rebuild του frontend container.** Βλ. #3 παραπάνω — τα #1 και #4 δεν έτρεξαν ποτέ. **Κανόνας: frontend αλλαγές ⇒ `docker compose up -d --build frontend` πριν από κάθε ισχυρισμό ότι κάτι δουλεύει.**

### Files changed
- Backend: `users.service.ts` (+spec), `auth.service.ts` (+spec), `users.e2e-spec.ts`, `auth.e2e-spec.ts`, `schema.prisma`
- Frontend: `tooltip.tsx` (νέο), `dialog.tsx` (+`DialogBody`), `alert-dialog.tsx`, `datetime.ts` (+2 helpers), `ShiftForm.tsx` (+spec), `EmployeeForm.tsx`, `EmployeeList.tsx`, `TeamPage.tsx` (+spec), `ShiftHistoryPage.spec.tsx`, `NewCodeDialog.tsx` + `ReactivateEmployeeDialog.tsx` (νέα), `messages.ts`, `App.tsx`
- Context: `architecture.md` (invariant), `swifttrack-phase1-final.md`

### ⚠️ Ένα commit, όχι πέντε
Ο χρήστης ζήτησε ενοποίηση. Τα πέντε commits έγιναν squash με `git reset --soft` (ήταν **μόνο τοπικά** — το remote ήταν στο `06a5211`, επαληθεύτηκε με `ls-remote` πριν το rewrite· κρατήθηκε και κλάδος `backup-before-squash`). Ο κύκλος «#3 μπήκε → βγήκε → ξαναμπήκε διορθωμένο» εξαφανίζεται έτσι από το ιστορικό, που είναι και το σωστό: στο δέντρο μένει μόνο η **δουλεύουσα** εκδοχή.

⚠️ **Και ο χρήστης ζήτησε ρητά να μην γίνονται commits χωρίς ερώτηση.** Είχε ήδη καταγραφεί ως προτίμηση και δεν τηρήθηκε.

### Verification
- Backend unit **253/253**, e2e **130/130** (ήταν 127· +3 νέα).
- Frontend **251/251** σε 17 αρχεία, καθαρό run μόνο του.
- ⭐ **Το layout μετρήθηκε, δεν υποτέθηκε.** Headless Chrome (`--headless=new --dump-dom`, υπάρχει ήδη στο μηχάνημα, **καμία εγκατάσταση**) πάνω σε σελίδα-probe με τα ίδια CSS. Το Save έμεινε **πλήρως ορατό σε 317 / 397 / 564 / 697 / 797px**, με όλα τα μηνύματα σφάλματος ταυτόχρονα· `topOffscreen=false` παντού, σκρολάρισμα μόνο όπου χρειάζεται. Το `SetupCodeDialog` (το μόνο `Dialog` χωρίς `DialogBody`) μετρήθηκε στα 265–283px σε παράθυρα 400/500/700 — **δεν κόβει ποτέ**, το cap είναι δίχτυ.
- ⚠️ **Αυτή είναι η διαδρομή που έλειπε.** Το `architecture.md` § Stack Trap #4 το έλεγε ήδη: *το jsdom δεν υλοποιεί layout· ό,τι εξαρτάται από πραγματικές διαστάσεις ανήκει στο Playwright, όχι σε Vitest spec*. Μέχρι το 13b, **το headless Chrome καλύπτει το κενό**.
- ⚠️ Ένα run ανέφερε «16 files / 196 tests / 1 error» — το **δέκατο πέμπτο** σφάλμα μέτρησης, ίδιο μοτίβο: αρχεία που δεν πρόλαβαν worker. Καθαρό run → 17/17. **Ποτέ παράλληλα με άλλο βαρύ run.**
- `tsc --noEmit` + `eslint` καθαρά και στα δύο.
- ⛔ **`payroll.e2e-spec.ts` πράσινο ΧΩΡΙΣ καμία τροποποίηση** — η απόδειξη ότι η τιμολόγηση δεν κουνήθηκε.
- ⭐ **Ζωντανά**: Anna → στήλη **2450**, badge → 2400 (ήταν 2400/2400) · μετονομασία **διατήρησε** την αύξηση · δεύτερη αύξηση ανέφερε σωστά 2450 ενώ η στήλη κρατούσε 2400 · login προφίλ **2450** · payroll **αμετάβλητο** σε 3 κύκλους.
- Τα δοκιμαστικά δεδομένα επαναφέρθηκαν πλήρως (όνομα, τιμή, γραμμές `UserRate`).

### ⚠️ Open
1. **13b (Playwright)** — το τελευταίο ανοιχτό του build plan. ⭐ Θα είχε πιάσει το #3: είναι ακριβώς το είδος ελέγχου που έλειπε.
2. **§13 gap 7** και **gap 8**.
3. ⚠️ **Ο picker εξακολουθεί να ΜΠΟΡΕΙ να σκεπάζει το Save — άγνωστο, όχι επιλυμένο.** Τα χωριστά πεδία μίκρυναν το popup (λίστα ωρών αντί ημερολογίου) αλλά το κενό End→Save παραμένει **152px**. Το ύψος του `time` popup **δεν μετριέται από JS** — είναι browser chrome, εκτός DOM· ούτε screenshot το πιάνει σε headless. Χρειάζεται **οπτικό έλεγχο του χρήστη**. Τα «~320px / ~200px» σε αυτή την καταχώρηση είναι **εκτιμήσεις**.
...
## Step 18 — Audit log (backend)
Status: ✅ Done
Date: 2026-09-12

Το παλαιότερο καταγεγραμμένο κενό του project (spec §4 decision 1a → §13 gap 2), δηλωμένο ως επόμενο βήμα σε **τρία** σημεία. Κλείνουν **και τα δύο** κενά που τα context αρχεία επικαλούνταν εναλλάξ: οι **ώρες** (§13 gap 2 — τίποτα δεν κατέγραφε ότι μια βάρδια άλλαξε ή σβήστηκε) και οι **διοικητικές πράξεις** (8g — τίποτα δεν κατέγραφε ποιος admin έκανε reset σε ποιον).

### Απόφαση (χρήστη)
| Ερώτημα | Απόφαση |
|---|---|
| Εύρος | **Ώρες + διοικητικές πράξεις** — ένας πίνακας, ένα βήμα |
| Ανάγνωση | **Χωρίς endpoint, χωρίς σελίδα** — SQL· το UI ξεχωριστό βήμα |
| Αν αποτύχει η εγγραφή | **Ίδιο transaction** — γυρίζει πίσω και η μεταβολή |
| Περιεχόμενο | **before + after με allowlist** — ποτέ ολόκληρη η γραμμή |
| Failed logins | **Όχι** — μόνο επιτυχημένες μεταβολές (τα failures τα έχει το Step 17) |

### ⭐ Το εύρημα που καθόρισε τη δουλειά
**8 στις 13 διαδρομές εγγραφής δεν έπαιρναν ποτέ τον actor.** Το `TimeEntriesService` είχε ήδη `caller: { userId, role }`· ολόκληρο το `users` module και το `PUT /settings` έπαιρναν **μόνο το υποκείμενο** (`@Param('id')`). Χωρίς το threading, ο πίνακας θα έλεγε «άλλαξε ο μισθός» αλλά όχι **ποιος** τον άλλαξε — δηλαδή ακριβώς το ερώτημα του 8g. Κόστος: 7 controllers + 7 υπογραφές services.

### ⭐ Το append-only είναι trigger, όχι REVOKE — **μετρήθηκε**
Η προφανής λύση δεν κάνει τίποτα εδώ: η εφαρμογή συνδέεται ως ρόλος που **και κατέχει** τον πίνακα **και είναι superuser**, οπότε τα table privileges δεν συμβουλεύονται καν — το `UPDATE` **πέτυχε** με το `REVOKE` σε ισχύ. Ένα `BEFORE UPDATE OR DELETE` trigger δεσμεύει ακόμη και superuser-owner. Χωρίς αυτή τη μέτρηση το βήμα θα είχε δηλώσει μια εγγύηση που δεν υπήρχε.

⚠️ Το `TRUNCATE` **σκόπιμα δεν φράσσεται** (δεν το συμβουλεύεται row-level trigger): το `test/helpers/db.ts` το χρειάζεται ανάμεσα στα e2e, και μια πολιτική διατήρησης θα το χρειαζόταν. Η εγγύηση είναι ότι **καμία μεμονωμένη γραμμή** δεν αλλοιώνεται ή σβήνει· το άδειασμα όλου του πίνακα είναι εμφανές.

### Files added/changed
- `prisma/schema.prisma` — `AuditLog` + `enum AuditAction` (13 τιμές)· **+ διόρθωση** του σχολίου `tokenVersion` που έλεγε «only change-password» και ήταν λάθος από το 8g.
- `prisma/migrations/20260911202946_add_audit_log/` — πίνακας, enum, 2 indexes, **+ χειρόγραφο trigger** append-only με την αιτιολόγηση της μέτρησης.
- `src/audit/audit.module.ts` — ⚠️ **δεν** κάνει import `PrismaModule`: δεν υπάρχει client για να γράψει κανείς εκτός του transaction του καλούντος.
- `src/audit/audit.service.ts` — `record(tx, entry)` / `recordAll(tx, entries)`.
- `src/audit/audit-entry.types.ts` — το allowlist ως discriminated union + 4 mappers.
- `src/audit/audit.service.spec.ts` — 11 tests.
- `test/audit.e2e-spec.ts` — 15 tests.
- `src/users/{controller,service,module}.ts` — actor threading σε 6 routes, transactions, audit· νέο ιδιωτικό `writeAndAudit()` (4 μέθοδοι είχαν το ίδιο σχήμα).
- `src/settings/{controller,service,module}.ts` — actor threading + audit.
- `src/time-entries/{service,module}.ts` — audit σε create/update/remove (**καμία** αλλαγή υπογραφής — είχε ήδη τον actor).
- `src/app.module.ts`, `test/helpers/db.ts` (TRUNCATE), 3 υπάρχοντα spec.
- Context: **και τα 4** — spec (§3 νέος πίνακας, §13 gap 2 ✅, decision 1a), `architecture.md` (4 νέα invariants + **3 διορθώσεις** παλιών ισχυρισμών), `build-plan.md` (Step 18), αυτό.

### ⭐ Τέσσερις μεταλλάξεις, όλες εκτελεσμένες, όλες κόκκινες
| # | Μετάλλαξη | Αποτέλεσμα |
|---|---|---|
| 1 | Το allowlist γίνεται `{ ...row }` | **2 unit κόκκινα** ✅ (μαζί το leak test) |
| 2 | Αφαίρεση του no-op guard στο `reactivate` | **1 e2e κόκκινο** ✅ |
| 3 | Το delete βγαίνει από το transaction | **1 e2e κόκκινο** ✅ — η βάρδια χάθηκε **χωρίς** εγγραφή |
| 4 | `resetPassword(id, id)` — ο actor γίνεται το υποκείμενο | **1 e2e κόκκινο** ✅ |

⚠️ Η **4η** είναι η σημαντική: τίποτα στο *σχήμα* της γραμμής δεν προδίδει την αντιμετάθεση (και οι δύο στήλες είναι `Int?`), άρα μόνο ένα τεστ που ξέρει **ποιος** έπρεπε να είναι ο actor μπορεί να την πιάσει.

### Verification που εκτελέστηκε
- Backend unit **275/275** (264 → **+11**), e2e **152/152** (137 → **+15**). ⭐ **Οι βάσεις μετρήθηκαν πριν αρχίσω** (264/137), δεν αντιγράφηκαν — το μοτίβο των 16 σφαλμάτων μέτρησης του project. Ταίριαξαν με τον tracker.
- `tsc --noEmit` + `eslint --max-warnings=0` καθαρά.
- ⭐ **Ζωντανά** (`docker compose up -d --build backend`), με πραγματικά μυστικά:
  - υπάλληλος 46, αύξηση 2450→2800, **δεύτερη** αύξηση →3000, reset password, deactivate, reactivate, reactivate ξανά.
  - ⭐ **Η υπερκαλυφθείσα τιμή 2800 σώζεται στο `before`** — το `UserRate` upsert την είχε σβήσει, υπάρχει **πουθενά αλλού** στη βάση.
  - ⭐ `PASSWORD_RESET_BY_ADMIN` με `actorId=1`, `subjectId=46` — **η γραμμή που το 8g είπε ότι δεν μπορούσε να γράψει**.
  - ⭐ Το **τρίτο** reactivate (no-op) έγραψε **καμία** γραμμή· τα δύο πραγματικά έγραψαν από μία.
  - ⭐ **Secret sweep: 0** για setupCode `9748`, για `$2b$`, για τις λέξεις password/setupCode/tokenVersion, για email.
  - ⭐ `UPDATE`/`DELETE` στον πίνακα → **και τα δύο ERROR** «AuditLog is append-only».
  - ⭐ **0 γραμμές `[Request]`** για αυτές τις επιτυχημένες εγγραφές — η απόδειξη ότι το Step 17 όντως δεν κάλυπτε αυτό το κενό.
  - ⭐ Διαγραφή του υπαλλήλου → **και οι 6 audit γραμμές επέζησαν** (καμία FK, εσκεμμένα).

### ⚠️ Διορθώσεις παλιών ισχυρισμών (doc/κώδικα)
Ίδιο μοτίβο με το Step 16 και το Step 17 — γράφονται επί τόπου, δεν σβήνονται:
1. `architecture.md`: «**No audit trail**» (invariant του 8g) → ✅ κλειστό, με την ιστορία του να παραμένει.
2. `architecture.md`: «η **μόνη** `$transaction` του codebase» → **13** πλέον· και άλλαξε **σχήμα** (array → callback).
3. `schema.prisma`: «only change-password bumps `tokenVersion`» → **και** το `resetPassword` (λάθος από το 8g).

### ⚠️ Ανοιχτά
1. **Χωρίς UI/endpoint ανάγνωσης** — απόφαση του χρήστη, όχι παράλειψη. Οι γραμμές διαβάζονται με SQL. Ξεχωριστό βήμα αν χρειαστεί.
2. **Χωρίς πολιτική διατήρησης** — ο πίνακας μεγαλώνει για πάντα. Σε αυτή την κλίμακα (λίγοι χρήστες) δεν είναι πρόβλημα· η πόρτα (`TRUNCATE`) υπάρχει και είναι τεκμηριωμένη.
3. **13b (Playwright)** και **§13 gaps 7, 8** — αμετάβλητα.
4. **Frontend observability** — αμετάβλητο από το Step 17.

### Follow-up από `/review` (ίδια μέρα)

Το review βρήκε **3 ευρήματα** — 1 σημαντικό, 2 μικρά, **κανένα κρίσιμο**. Διορθώθηκε το σημαντικό· τα δύο μικρά καταγράφηκαν συνειδητά αντί να «φτιαχτούν».

⭐ **Σημαντικό — φάντασμα `EMPLOYEE_UPDATED`.** Το `updateEmployee` έγραφε γραμμή audit όταν **δεν είχε αλλάξει τίποτα**. Μετρήθηκε ζωντανά, δεν συμπεράθηκε:

```
 9 | EMPLOYEE_UPDATED | before_name: Review Probe | after_name: Review Probe
                      | before_rate: 2450         | after_rate: 2450
```

Οι δύο κλάδοι της **ίδιας μεθόδου** διαφωνούσαν: ο κλάδος αλλαγής μισθού έλεγχε inline `dto.name !== employee.name`· ο κλάδος χωρίς αλλαγή μισθού έγραφε **χωρίς όρο**. Ο έλεγχος γράφτηκε εκεί όπου το πρόβλημα **επιβαλλόταν** στην προσοχή (ο κλάδος που μπορεί να βγάλει δύο γεγονότα έπρεπε να αποφασίσει ποιο συνέβη) και δεν γύρισε πίσω στον άλλον.

⚠️ **Δεν είναι υποθετικό**: το `EmployeeForm` στέλνει **και τα δύο πεδία σε κάθε save**, άρα «άνοιξε υπάλληλο, πάτα Save» είναι η **συνηθισμένη** διαδρομή. Σε πίνακα που υπάρχει για να τον εμπιστεύεσαι, γραμμές χωρίς αντίστοιχη αλλαγή κάνουν όλες τις υπόλοιπες λιγότερο πιστευτές.

⚠️ **Παραβίαζε invariant που είχε γράψει το ίδιο αυτό βήμα** («a no-op writes nothing») — η οξύτερη μορφή απόκλισης doc/κώδικα που το project συνεχίζει να βρίσκει: ο κανόνας δηλώθηκε και σπάστηκε 300 γραμμές παρακάτω.

⚠️ **Οι 4 μεταλλάξεις δεν μπορούσαν να το πιάσουν.** Μια μετάλλαξη σπάει guard που **υπάρχει**· δεν αποκαλύπτει guard που **λείπει**. Αυτό είναι το επιχείρημα γιατί το review pass είναι ξεχωριστό από το spike pass και όχι αντίγραφό του.

**Η διόρθωση**: το `nameChanged` υπολογίζεται δίπλα στο `rateChanged` στην κορυφή της μεθόδου — **μία απάντηση, και οι δύο κλάδοι**. Το UPDATE εξακολουθεί να εκτελείται σε κάθε περίπτωση: απόφαση audit δεν επιτρέπεται να αλλάξει τι απαντά το endpoint. Όταν δεν άλλαξε τίποτα δεν ανοίγει καν transaction — δεν υπάρχει τίποτα να ζευγαρώσει.

| Σενάριο | Πριν | Μετά |
|---|---|---|
| Πανομοιότυπο re-save | ❌ 1 φάντασμα | ✅ **0** |
| Αύξηση + ίδιο όνομα | ✅ μόνο RATE_QUEUED | ✅ μόνο RATE_QUEUED |
| Μόνο μετονομασία | ✅ EMPLOYEE_UPDATED | ✅ EMPLOYEE_UPDATED |

**Τα 2 μικρά, συνειδητά ΧΩΡΙΣ διόρθωση:**
- **Δύο συνδέσεις pool ανά εγγραφή** — το `writeAndAudit` μέσα σε `Promise.all` με το `findRateNow` κρατά transaction + query ταυτόχρονα, σε default pool 10 (~5 ταυτόχρονα writes). Επαρκές σε αυτή την κλίμακα· η σειριοποίηση θα κόστιζε **σίγουρο** round trip σε 6 endpoints για όριο που κανείς δεν πλησιάζει. Καταγράφηκε στο `architecture.md` ώστε μια μελλοντική αύξηση προσωπικού να **βρει** τον αριθμό αντί να τον ξανα-ανακαλύψει.
- **Το κενό tests** — καλύφθηκε από τη διόρθωση παραπάνω.

### Verification μετά τα fixes
- Backend unit **277/277** (275 → +2), e2e **154/154** (152 → +2). `tsc --noEmit` + `eslint --max-warnings=0` καθαρά.
- ⭐ **Μετάλλαξη #5**: `nameChanged` → `dto.name !== undefined` → **2 unit ΚΑΙ 2 e2e κόκκινα** ✅, επαναφορά επιβεβαιωμένη με `diff`.
