...
Δεν προκαλεί 500 (η epoch γραμμή το εγγυάται). ⚠️ **Το §13 gap 8 έχει πλέον ανακριβή περιγραφή**: λέει «slips a queued raise by one cycle», ενώ τώρα η ειδοποίηση **εξαφανίζεται**. Η λύση που το ίδιο το §13 προτείνει (ευθυγράμμιση μελλοντικών `UserRate` γραμμών μέσα στο `updateSettings`, ένα transaction, μόνο `effectiveFrom > now`) καλύπτει και τις δύο εκδοχές.
