...
| role | `Role` | |
| hourlyRate | `Int?` | EMPLOYEE only· ISK, always an integer (no decimals)· the admin never clocks in/out, so never needs one. ⚠️ This is what the employee is paid **now** — the denormalised head of `UserRate`, read by `/users`, `/users/me` and the login response, none of which is a cycle-aware question. **Payroll never reads it**: the rate that prices a cycle comes from `UserRate`. The two are written in one transaction and cannot disagree |
