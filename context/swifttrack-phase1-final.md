...
| 8 | **Moving the cycle boundary slips a queued raise by one cycle** | `UserRate.effectiveFrom` is an instant, computed from the `cycleStartDay` in force when the raise was entered· lookups compare it against the *current* boundary. Move the boundary in between (25 → 15, say) and that instant lands mid-cycle, so the `<=` test misses it and the raise takes effect a cycle later than intended. Silent — nothing warns, and the `ChangeCycleDialog` copy ("No shift is altered") is true of `TimeEntry` and says nothing about rates. ⚠️ It does **not** cause a 500: `createEmployee` writes every employee an epoch row, which precedes any possible cycle start, so "no rate in force" stays unreachable without hand-editing the database. **Accepted deliberately** — `cycleStartDay` is configured once at setup and then left alone, so the collision needs a boundary change *while* a raise happens to be queued | Realign future `UserRate` rows onto the new boundary inside `updateSettings`, in one transaction — only rows with `effectiveFrom > now`, so no settled cycle is touched. Cheap to build· it is the *occasion* that does not arise, not the fix that is hard |
