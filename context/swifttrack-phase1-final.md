...
| role | `Role` | |
| hourlyRate | `Int?` | EMPLOYEE only· ISK, always an integer (no decimals)· the admin never clocks in/out, so never needs one. ⚠️ The **column** is the newest rate *entered*, written the moment an admin saves — while the `UserRate` row it pairs with is dated to the next cycle, so during a queued raise the two differ by design. **No response DTO serves it**: the API field `hourlyRate` on `/users`, `/users/me` and the login response means what the employee is paid **now**, resolved from `UserRate` by `findRatesNow` (restored after step 15b, once the code had drifted to serving the column). **Payroll never reads it** either: the rate that prices a cycle comes from `UserRate`. The column's remaining job is answering "did this request change the rate?" |
...
| rates | `UserRate[]` | relation |

### UserRate

The employee's pay history, and the only thing payroll prices a cycle with. Pay is derived from raw shifts on every request, so without a record of what the rate *used to be*, a raise would silently reprice every cycle the employee had ever worked — including ones already paid.

| Field | Prisma | Notes |
|---|---|---|
| id | `Int @id @default(autoincrement())` | |
| hourlyRate | `Int` | **not** nullable, unlike `User.hourlyRate` — a row exists precisely *because* there is a rate. An ADMIN never gets one |
| effectiveFrom | `DateTime` | the instant this rate starts applying. Always a **cycle-start instant** (UTC midnight), i.e. a value `computeCycleRange().start` can produce |
| userId | `Int` | |
| createdAt | `DateTime @default(now())` | |
| user | `User @relation(fields: [userId], references: [id])` | |

`@@unique([userId, effectiveFrom])` — two rates in force from the same instant is not a state with an answer. Also the key the rate write upserts on, so two raises inside one cycle collapse into a single row for the next one instead of colliding.

`@@index([userId, effectiveFrom])` — every lookup is "the greatest `effectiveFrom` at or before this instant".

**Why an instant and not a cycle key.** A stored `"2026-07"` would need `cycleStartDay` to interpret, so an admin moving the boundary from the 25th to the 15th would silently re-interpret **every** stored row and move history again — the exact failure this table exists to prevent. Same discipline as `cycleEnd` vs `endExclusive`: store the unambiguous thing.

**The first row of every employee starts at the epoch**, written by `POST /users` in the same statement as the user. An admin may write a shift at any past date (§7a rule 5), so a cycle earlier than the employee's own row is reachable — and a cycle with no rate in force makes payroll fail loudly, which on the team overview would take down the page for everyone. Anchoring at the epoch makes "no rate in force" unreachable for anybody who has one, so that 500 keeps meaning what it was written to mean: somebody edited the database by hand.
...
> No rate and no pay amount either: a shift records **when** somebody worked, never what it was worth. Pricing is resolved per *cycle* from `UserRate` (§4, decision 5g), not frozen per shift — a rate stamped on each row would have to be corrected row by row when it was wrong.

### AppSettings
| Field | Prisma | Notes |
|---|---|---|
| id | `Int @id @default(1)` | fixed, always `1` — a single unique row |
| cycleStartDay | `Int @default(25)` | allowed range **20–25** (was 11–25 until 2026-09-12) — enforced by `class-validator` on the DTO **and** by a database `CHECK` (see §4, decision 5a). The only field the cycle arithmetic reads |
...
Who changed what, and what it said before. Closes §13 gap 2. Append-only: rows are inserted and never altered.

| Field | Type | Why |
|---|---|---|
| id | `Int @id @default(autoincrement())` | |
| action | `AuditAction` | The domain act, not the HTTP verb or the table touched — a reader asking "who reset this password" is not asking which row received an UPDATE. 13 values, imported from the generated client like `Role` |
| actorId | `Int?` | Who acted. `null` **only** for `ACCOUNT_ACTIVATED`, the single mutation performed without a session — there the actor is the subject by construction, and writing the subject into both columns would invent a fact |
| subjectId | `Int?` | Who it was done to. `null` for `SETTINGS_UPDATED`, which has no subject: global configuration is not done *to* anybody. ⚠️ Separate from `actorId` because the pair **is** the question — 8g's gap was never "the password changed" but "which admin changed whose password" |
| entityType | `String` | `"TimeEntry"`/`"User"`/`"UserRate"`/`"AppSettings"`. A string, not an enum: it names a table, and a second enum tracking the first buys nothing |
| entityId | `Int?` | `null` for the `AppSettings` singleton, whose id is always 1 and carries no information |
| before | `Json?` | The allowlisted fields as they stood. `null` on a create |
| after | `Json?` | …and as they became. `null` on a delete — which is what makes a **deleted shift's hours recoverable from this table and nowhere else**, `DELETE /time-entries/:id` being the only hard delete in the backend |
| createdAt | `DateTime @default(now())` | |

⚠️ **`before`/`after` are built from an explicit per-entity allowlist, never by spreading a row** — `TimeEntry`: startTime/endTime/notes· `User`: name/hourlyRate/isActive· `UserRate`: hourlyRate/effectiveFrom· `AppSettings`: both cycle days. The alternative (snapshot the row, strip the secrets) is a *blocklist*, which architecture.md already refuses for request logging: a list to maintain at every future column whose failure mode is silent. `password`, `setupCode`, `setupCodeExpiresAt` and `tokenVersion` are never values here — a revocation is recorded as an **action**.

⚠️ **No relation to `User`.** `actorId`/`subjectId` carry no foreign key, deliberately: a relation makes these rows reachable by cascade, and a trail that can be deleted by deleting its subject is not one. Verified live — deleting an employee leaves their audit rows intact.

⚠️ **DB-level, hand-written** (`..._add_audit_log`): a `BEFORE UPDATE OR DELETE` trigger raising `restrict_violation`. **Not `REVOKE`** — measured against the running database, the app's role both owns the table and is a superuser, so table privileges are not consulted and the UPDATE succeeded with the REVOKE in place. `TRUNCATE` is deliberately **not** blocked (a row-level trigger is not consulted by it): the guarantee is that no individual row can be altered or removed, while emptying the table is conspicuous and is the door a retention policy would need.
...
| 1a | Audit log for deletions/edits — ✅ **built in step 18** (2026-09-12), see §13 gap 2 | Was not present in core — and the same call was made for an **approval flow** (`source`/`status`/`approvedById`), weighed when `POST /time-entries` was added and deferred for the same reason: both need domain-model changes, so both belong to a later phase. ⚠️ The consequence recorded here — "an employee's self-reported hours carry no history, and nothing records that an entry was edited or deleted" — **no longer holds**: step 18 added the `AuditLog` table (§3). The **approval flow** half of this decision is untouched and remains deferred (§13 gap 3): recording that an entry was edited is not the same as requiring somebody to approve it |
...
| 5a | Allowed cycle days | `cycleStartDay` is restricted to **20–25**, and `cycleEndDay` is always exactly `cycleStartDay - 1` (so 20–19 … 25–24) — any other combination is rejected with a 400. ⚠️ **Narrowed from 11–25 on 2026-09-12**, and the reason is a **business rule** rather than a technical one: a pay cycle starts at the end of the month, and days 11–19 were theoretical flexibility nobody used. The narrowing came with a migration that moves any stored day below 20 to 25, because a row outside the new range keeps working while locking the admin out of the only screen that could repair it. It also added two DB `CHECK` constraints (the range, and `cycleEndDay = cycleStartDay - 1`), so the rule now holds below the application as well — the same layering as the `AppSettings` singleton check. Two consequences, both deliberate: consecutive cycles are contiguous, so no shift can fall into a gap between them or into two at once; and every allowed day exists in every month, so no day-of-month clamping for 28/29/30/31-day months is ever needed. Since the range is fully determined by `cycleStartDay`, the cycle arithmetic uses **only** that field — `cycleEndDay` is stored and validated but never computed with |
...
| 5g | A raise applies forward — one rate per cycle | A cycle is priced with the rate **in force at its start**, read from `UserRate` and resolved **once per cycle** — never per day, per shift or "as of now". Pay is recomputed from raw shifts on every request, so a rate stored as a single current value would make every raise reprice the employee's entire history, cycles already paid included. ⚠️ **Anchored at the cycle start, never prorated within a cycle.** A raise entered on 5 August with a 25th boundary leaves cycle `2026-07` (25 Jul – 24 Aug) entirely on the old rate and takes effect on 25 August. Prorating was considered and rejected on two structural grounds, not taste: it partitions the cycle by rate period, turning four money roundings into eight and breaking decision 5d· and the summary shows **one** rate per zone, so it would need either two rows per zone or a "blended" rate that no longer reproduces its own Pay column when multiplied by the hours — exactly what 5d's never-round-the-rate rule exists to guarantee. One cycle, one rate, is what holds the rounding surface at three points. The admin does **not** choose the date: `PUT /users/:id` takes `name` and `hourlyRate` as before and "from the next cycle" is implicit· the Team form states it in a permanent line, the way Settings already states what moving the cycle boundary does. ⚠️ **The queued raise is reported and shown, not left implicit.** Between entering it and the cycle opening — up to a month — `User.hourlyRate` and the rate pricing the current cycle are legitimately different numbers, and without saying so the Team list shows a figure no payslip uses while an employee sees a larger rate on their profile than on their own payroll. So both `GET /users` and `GET /payroll` carry `pendingRate`/`pendingRateEffectiveFrom`, and both screens print it (§8). Note the two answer the same question against different reference points: the Team list has no cycle, so pending there means "not yet in effect"· payroll means "**at the end of this cycle**" — the raise the reader has not been paid at yet. ⚠️ **Exactly one cycle announces a raise, the one immediately before it** (tightened in step 15b): payroll matches `effectiveFrom` against the cycle's own end, not "any time after this cycle". The looser reading shipped first and printed the same future-tense line on every earlier cycle too, including settled ones that had already been paid — a cycle two raises back has no gap between its price and its payslip, so it says nothing |
...
| PUT | `/users/:id` | ADMIN | Edit (name, hourlyRate). A **changed** rate takes effect from the **start of the next cycle** — the cycle in progress and every past one keep the rate they were already priced at (§4, decision 5g). No effective-date field: the same rate resubmitted writes no history at all (the form always sends both fields), and two raises inside one cycle collapse into one, so a typo stays correctable until that cycle begins. The queued change comes back as `pendingRate`/`pendingRateEffectiveFrom` — **including on an edit that changed no rate**, since a rename must not hide a raise that is still coming |
...
| POST | `/users/:id/reset-password` | ADMIN | Clears the password and issues a fresh 4-digit code with a new 3-day expiry, so the employee sets a new password exactly as at account creation. **Added in step 8g**, for the case neither other password route reaches: `PATCH /auth/change-password` needs the *current* password, and `reset-setup-code` refuses once the account is activated — so an employee who simply forgot their password had no remedy short of editing the database. Unlike `reset-setup-code` it has **no guard on activation state**: it also succeeds on a still-pending row (same outcome, so refusing would only send the admin to the other endpoint) and on a deactivated one (without reactivating them — `set-initial-password` checks `isActive` first, so the reset stays inert until `reactivate`). Revokes every token already issued for the account, reusing step 8f's `tokenVersion`. EMPLOYEE rows only, like `PUT`/`DELETE`/`reactivate` |
...
| PUT | `/settings` | ADMIN | Update cycleStartDay/cycleEndDay. Both fields are required· `cycleStartDay` must be 20–25 and `cycleEndDay` must be exactly `cycleStartDay - 1`, otherwise 400 (see §4, decision 5a) |
...
hourlyRate  = the UserRate in force at cycleStart     (one rate for the whole cycle — 5g)
...
An employee with **no `UserRate` in force at the cycle's start** fails **loudly** with a 500 naming the fix — never a silent 0, which would quietly drop that person's wages out of the team's total cost. `POST /users` requires a positive rate and writes the first row at the epoch, so every cycle however old has one, and this can only arise from rows edited directly in the database.
...
| Action on an **activated, active** employee's row | **Reset password** (step 13-5) — clears a forgotten password and issues a fresh code. ⚠️ Hidden on a pending row, where `New code` beside it already does exactly this, and on a deactivated row, where the code stays inert until `Reactivate`. Unlike `Reactivate` above, the reason is **not** that the action would fail — the API accepts it on any employee row |
...
| Standing text on `/login`, under the activation link (step 13-5). ⚠️ **Always visible, never a response to a failed sign-in**: `login` answers `INVALID_CREDENTIALS` for an unknown email and a wrong password alike, so a hint shown only after a failure could not know which case it was answering — and someone who has forgotten their password needs it *before* guessing. It names the **activation code** because that is what ties it to the link beneath it and to the four digits the admin later reads out· nothing in that journey is automated | — | "Forgot your password? Ask your admin to reset it — they'll give you a new activation code." |
...
**Team** (step 13-3). The page title and the badges/toggle/actions are in the binding tables above· everything here is the client's own and may be improved without a spec update:

- The setup-code dialog, opened after a create (**Activation code**), after a re-issue (**New activation code**) and after a password reset (**Password reset — new activation code**, step 13-5) — one component, three call sites. Body: *"Give this code to &lt;name&gt;. It is the only way they can sign in for the first time, or again after a password reset."* ⚠️ The third title names the *reset* rather than repeating "activation code", because the admin arrived from a different action and the title is their only confirmation that it succeeded. ⚠️ The body is **one string for all three**, so it has to fit both a first sign-in and a return after a reset — it originally ended at "for the first time", which 13-5 made false for an employee who had been signing in for months.
- The expiry, beneath it and on every pending row: *"Valid until 29 Aug 2026."* ⚠️ A **date**, not a duration — "3 days" is arithmetic against a calendar the reader may not be looking at, while a date can be written down beside the code. It is produced by the existing `formatDate` in `lib/datetime.ts`, which is why it carries the **year**: this line originally read *"Valid until 29 August"*, and a fifth date formatter for one string was not worth a fourth format in the app. The year is noise over a 3-day life, but it is not wrong.
- The deactivation confirmation: *"Deactivate this employee?"* / *"&lt;name&gt; will no longer be able to sign in, and their row moves behind “Show deactivated”. Their shifts and payroll history are kept, and you can reactivate them here at any time."* ⚠️ It says the row **moves** and that the action **can** be undone — both load-bearing. The build-plan's draft said "cannot be undone from the app", which `PATCH /users/:id/reactivate` (step 8c) makes false, and which would have been contradicted by the `Reactivate` button on the very row in question.
- The password-reset confirmation (step 13-5): *"Reset this employee's password?"* / *"&lt;name&gt; will not be able to sign in until they set a new password, and any device they are signed in on will be signed out. You'll get a new activation code to give them."* ⚠️ All three clauses carry something the admin can see nowhere else: this is **not** a "send them a reset link" flow, the sign-out is real and immediate (8f's token revocation), and the job is **not finished** when the dialog closes — a code still has to reach the employee out of band.
- The one toast on the page: *"&lt;name&gt; has been deactivated. Their row is under “Show deactivated”."* The other six writes take none — see architecture.md § Invariants for the rule and for why deactivate is the exception. ⚠️ **Reset password** takes none for a sharper reason than create's: it is not complete when the request returns, only when the code reaches the employee, so a toast would announce success at the halfway point· the code dialog is what the admin needs instead.
- Two empty states, deliberately distinct. *"No employees yet."* (reused from step 13-1) means the roster really is empty· *"Every employee is deactivated. Turn on “Show deactivated” to see them."* means the **filter** emptied the table, where the first sentence would be false and would point an admin at "create one" while the people they want sit one toggle away.
- Field validation for `EmployeeForm`: *"Enter a name."* · *"Enter an hourly rate."* · *"The hourly rate must be a whole number of at least 1 ISK."* ⚠️ The middle one exists because an empty `<input type="number">` read with `valueAsNumber` yields **`NaN`**, not `undefined`, and `z.number()` would otherwise report "expected number, received nan" to an admin who has simply typed nothing yet. ⚠️ *"Enter a name."* also answers a name of **nothing but spaces** — the schema is `z.string().trim().min(1)`, in that order, and the order is the rule: reversed, `"   "` passes the client and reaches the API as `""`, where a codeless `ValidationPipe` 400 surfaces as the generic *"Something went wrong."* (found and fixed in the step-13-3 review).
- Form labels: **Name** · **Hourly Rate (ISK)** · **Done** (dismisses the code dialog — not "Cancel", since nothing is being cancelled: the employee already exists by the time it opens). **Email** is reused from step 9 and appears **only when creating**.
...
|---|---|---|---|
| 1 | **Payroll never freezes** — ⚠️ narrowed, not absent | Pay is recomputed from raw shifts on every request, so it always reflects *current* data. **The rate is not part of that**: `UserRate` (§3, decision 5g) pins each cycle to the rate in force at its start, so a raise is forward-effective and settled cycles hold their value. What remains: **editing a shift** in a past cycle still changes that cycle's total, since an admin may write history at any date (§7a rule 5)· and a **zone percentage** change would still reprice everything, the factors having no history (decision 5c). Nobody is notified in either case, because nothing is stored to compare against. ⚠️ Do not read this row as "payroll is frozen" — it is not, and building on that premise would be wrong | A **payroll snapshot per closed cycle** — the hours, rates and amounts written down at the moment the cycle closes, and read back thereafter instead of recomputed. `UserRate` is forward-compatible with it rather than a substitute: the snapshot would store the resolved rate, which is already computed. It also happens to be the answer if the overview ever gets slow at a much larger headcount, since the figures would already exist |
| 2 | ~~**No audit log**~~ — ✅ **CLOSED in step 18** (2026-09-12) | ~~An employee's self-reported hours carry no history· nothing records that an entry was edited or deleted~~. The `AuditLog` table now records every successful mutation with its actor, its subject and the state before and after: manual shift create/update/delete (this row's original subject) and the administrative acts step 8g could not record from one endpoint (password reset, deactivate/reactivate, rate change, setup-code reissue, cycle boundary). ⚠️ **Two things it deliberately does not do**, so neither is rediscovered as a bug: clock-in/clock-out write no row (the `TimeEntry` *is* the record, written by the server's clock from the caller's own session), and there is **no read endpoint and no screen** — rows are read with SQL. A UI is a separate step. It is also **not** an approval flow (gap 3) and does not freeze payroll (gap 1) | ~~An append-only history table~~ — built exactly as described: append-only enforced by a Postgres trigger, since the app's role owns the table and `REVOKE` does not bind an owner (measured) |
...
| 7 | **An already-effective rate cannot be corrected through the API** — ⚠️ narrowed: the window is now **visible** | `PUT /users/:id` always writes the **next** cycle's rate (decision 5g), so a typo is freely correctable until that cycle starts and not afterwards. The correctable window is no longer invisible: `pendingRate` is shown on the Team row and on the payroll page, so an admin can see a queued raise and fix it before it lands rather than discovering it after. Once in force, the only remedy is editing `UserRate` by hand. Accepted deliberately: the obvious alternative — an effective-date field on the edit form — reopens the door 5g exists to close, since a backdated rate reprices a cycle that has been paid. Same shape as the gap `reset-setup-code` had before step 8c | An admin-only **supersede** path that is explicit about what it is: it names the cycle being repriced and is distinguishable in the UI from an ordinary raise, rather than a date field that makes rewriting history the accidental default |
| 8 | **Moving the cycle boundary slips a queued raise by one cycle — and, since step 15b, hides its announcement entirely** | `UserRate.effectiveFrom` is an instant, computed from the `cycleStartDay` in force when the raise was entered· lookups compare it against the *current* boundary. Move the boundary in between (25 → 15, say) and that instant lands mid-cycle, which now breaks **two** lookups rather than one. **(a) When it takes effect**: the `<=` test misses it, so the raise applies a cycle later than intended — the original finding. **(b) Whether it is announced** *(new in step 15b)*: `findEmployeeRateAt` matches `effectiveFrom === endExclusive`, and a mid-cycle instant equals no boundary, so `pendingRate` comes back **null** and the payroll page says nothing at all. ⚠️ This is a **worsening**, not a new bug, and it is worth stating plainly: the old `> at` test still surfaced the raise, merely on the wrong cycle. Noise became **silence**, which is the worse failure here — the notice exists precisely so the gap between the rate on the profile and the rate on the payslip is not read as an **underpayment**. Verified by execution, not inferred: with a row at `2026-09-25` and the boundary moved to 15, `> at` yields 3300 and `=== until` yields null. Silent on both counts — nothing warns, and the `ChangeCycleDialog` copy ("No shift is altered") is true of `TimeEntry` and says nothing about rates. ⚠️ It does **not** cause a 500: `createEmployee` writes every employee an epoch row, which precedes any possible cycle start, so "no rate in force" stays unreachable without hand-editing the database. **Accepted deliberately** — `cycleStartDay` is configured once at setup and then left alone, so the collision needs a boundary change *while* a raise happens to be queued | Realign future `UserRate` rows onto the new boundary inside `updateSettings`, in one transaction — only rows with `effectiveFrom > now`, so no settled cycle is touched. **One fix closes both halves**: put every queued row back on a real boundary and the `<=` test and the equality test are both satisfied again, which is the argument for realigning the data rather than loosening either lookup. ⚠️ Loosening the equality back to "any future rate" is **not** the fix — that is the step 15b bug, and it would trade a rare silence for a permanent falsehood on every past cycle. Cheap to build· it is the *occasion* that does not arise, not the fix that is hard |
