...
- [ ] **4. Settings module** *(swapped with Time Entries — see below)*
  - `GET /settings` (Both)
  - `PUT /settings` (ADMIN) — updates cycleStartDay/cycleEndDay. Both fields required· `cycleStartDay` 20–25 and `cycleEndDay` exactly `cycleStartDay - 1`, enforced by `class-validator` (custom `@IsDayBefore`), otherwise 400. See spec §4 decision 5a for why the range is restricted — contiguous cycles, and no day-of-month clamping ever needed
...
- [ ] **8a. Unit tests — cycle resolution, Payroll, Auth & Users**
  ...
  **Why this grew beyond cycle+payroll:** step 8 is a *snapshot* — it proves the backend worked on the day it was clicked through, not that it still works after the next change. That is not hypothetical here: step 3 rewrote `JwtStrategy` to hit the database on every request, a change that passes through **every** authenticated endpoint, and nothing automated would have caught it breaking login. `AuthService` in particular is the one place where a wrong order of checks is a security bug rather than a display bug — and it has already been wrong once (`set-initial-password` was missing the `isActive` check, caught by `/review` in step 3, not by any test).
  ...
  - `cycle.util.spec.ts` already exists from step 4 (boundaries, February, splitting, open shifts, prev/next keys, the omitted-`?cycle=` default) — extend rather than re-create. The 28/29/30/31 clamping cases from the original plan no longer apply: the 20–25 restriction means every allowed day exists in every month (spec §4 decision 5a)
...
- [ ] **8g. `POST /users/:id/reset-password`**
  ...
  **The endpoint.** `POST /users/:id/reset-password` (ADMIN), no request body — the target is the URL `:id`, same as `reset-setup-code`. Sits directly beside it in `UsersController`, and `UsersService.resetPassword(id)` sits beside `resetSetupCode`/`reactivate`/`deactivate`.
  ...
  - ⚠️ **No audit trail — an accepted gap, recorded rather than absorbed silently.** ✅ **Closed by step 18** (2026-09-12); the original wording is kept because the reasoning it gives is exactly the reasoning step 18 followed. No logging or audit-table infrastructure existed anywhere in this codebase at the time of 8g. No record was kept of which admin reset which employee's password, or when. A real audit log is a cross-cutting addition that does not belong hung off one endpoint· out of scope **here**, same as every other "recorded but not fixed" gap in this project (8c's fourth state, 8e's original non-revocation) — and built as its own step for precisely that reason, where the actor could be threaded through all thirteen write paths at once rather than one.
...
**Out of scope, explicitly**: the audit log (§13 gap 2) — it needs a new table, so it is a separate step under the AGENTS.md domain-model rule. ✅ Built as **Step 18** below.

---

## Step 18 — Audit log *(backend, done 2026-09-12)*

The project's oldest recorded gap, deferred since spec §4 decision 1a and named as the next step in three places. **Both** gaps that the context files cite interchangeably are closed here, deliberately as one step: §13 gap 2 (an employee's hours carried no history — nothing recorded that an entry was edited or deleted) and 8g's (nothing recorded which admin reset which employee's password). One table, one actor mechanism· splitting them would have threaded the actor through the same services twice.

**Step 17 narrowed the sentence but not the gap.** Operational logging writes a line per *failed* request and is deliberately silent on 2xx, and by invariant never reads a body — so a **successful** write left nothing at all. The two are different artefacts for different readers: the log answers *why did this request fail*, the audit log answers *who changed this, and what did it say before*. Verified live: the successful writes in this step's sweep produced **zero** `[Request]` lines.

**Scope, decided with the user**: hours **and** administrative acts· **no read endpoint and no screen** (rows are read with SQL — a UI is a separate step)· the audit row in the **same transaction** as the mutation· `before`/`after` from an **allowlist**· successful mutations only (failed logins stay step 17's business).

- ⭐ **The finding that shaped the work: 8 of 13 mutating routes never received the actor.** `TimeEntriesService` already took `caller: { userId, role }`; the whole `users` module and `PUT /settings` took only the **subject**. Without threading `@CurrentUser()` through seven controllers and seven service signatures, the table could say "the rate changed" but not who changed it — which is the entire question 8g asked.
- ⭐ **Append-only is a trigger, not `REVOKE` — measured, not assumed.** The app's role both owns the table and is a superuser, so table privileges are not consulted: an `UPDATE` succeeded with the `REVOKE` in place. A `BEFORE UPDATE OR DELETE` trigger binds even a superuser-owner. ⚠️ `TRUNCATE` is deliberately left open (a row trigger is not consulted by it) — `test/helpers/db.ts` needs it between e2e tests, and a retention policy would.
- **The allowlist is types, not filtering.** Snapshot-the-row-then-strip is a *blocklist* — the pattern architecture.md already refuses for request logging, because its failure mode at the next column is silent. ⚠️ The snapshots must be `type` aliases: Prisma's `InputJsonValue` wants an implicit index signature that an `interface` does not have.
- **Transactions: 1 → 13.** Only `updateEmployee` had one. `createEmployee` (nested write, needs the new id) and `updateEmployee` (array form, cannot depend on an earlier result) both moved to the **callback** form.
- **Written in the service, never a controller or interceptor** — `AuthService` writes `User` through `UsersService`, so a controller hook would miss activation and change-password. An interceptor never sees the row *before* the write, and could not join the transaction.
- **A no-op writes nothing**: `reactivate` on an already-active row is a documented 200 that still UPDATEs. **Clock-in/out write nothing** either — the `TimeEntry` is the record. Both pinned by tests, so the absence reads as a decision.

**Tests**: 264 → **277** unit, 137 → **154** e2e (baselines measured before starting, not copied). ⭐ **Five mutations, all executed and all red**: the allowlist replaced by a spread (2 unit fail, including the leak test), the `reactivate` no-op guard removed (the e2e written for it fails), the delete moved out of its transaction (the rollback test fails — the shift vanished with no record), the actor replaced by the subject (8g's own test fails, which is the one nothing about the row's *shape* would catch), and — added by the review below — `nameChanged` weakened to `dto.name !== undefined` (2 unit **and** 2 e2e fail).

### ⚠️ Follow-up from `/review` (same day)

The review found **one important issue**, and it is worth recording because of *how* it escaped: **`updateEmployee` wrote a phantom `EMPLOYEE_UPDATED` row when nothing had changed.** Measured against the running backend — an identical re-save produced a row whose `before` and `after` were byte-for-byte the same.

The two branches of one method disagreed: the rate branch guarded the name comparison inline, the no-rate branch wrote unconditionally. The guard was written where the problem was *forced* into view (the branch that can emit two events had to decide which one happened) and never carried back to the branch where it was not.

⚠️ **It violated an invariant this very step had just written** ("a no-op writes nothing"), which is the sharpest form of doc/code divergence this project keeps finding — the rule was stated and then broken three hundred lines away. ⚠️ **And the four mutations could not have caught it**: a mutation breaks a guard that exists· it cannot reveal one that is missing. That is the argument for the review pass being separate from the spike pass, not a duplicate of it.

Fixed by computing `nameChanged` beside `rateChanged` at the top of the method — one answer, both branches — with the write still issued in every case, since an audit decision must never change what the endpoint answers. Two unit tests and two e2e tests added, plus mutation #5 above. Re-verified live: an identical re-save now writes **nothing**, a raise with an unchanged name writes **only** `RATE_QUEUED`, and a rename writes `EMPLOYEE_UPDATED` with the two names differing.

Two minor findings were **deliberately not fixed**: the two-connections-per-write cost of `writeAndAudit` inside `Promise.all` (recorded as a known limit in `architecture.md` — serialising would cost a guaranteed round trip on six endpoints to buy a limit nobody is near), and the test gap itself, which the above closes.

⭐ **Live sweep against the running container**, the step 17 method applied to the table: a real employee, two raises, a reset, a deactivate/reactivate/reactivate. Zero hits for the setup code, any bcrypt hash, the words password/setupCode/tokenVersion, or an email. The superseded rate **2800** survives in `before` where `UserRate` has overwritten it. Deleting the employee left all six audit rows intact — the no-FK decision, in practice.

**Out of scope, explicitly**: a `GET /audit-log` endpoint and an admin screen· the approval flow (§13 gap 3 — recording an edit is not requiring approval of one)· payroll snapshots (gap 1).
